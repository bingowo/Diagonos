...
int main()
{
    ...
    if(out>1)printf("%lld ",(signd*sign<0)?(out-1)*sign:out*sign);
    else if(signd*sign>=0)printf("%lld ",out*sign);
    if(point!=n&&out)printf("%lld %lld",numerator,denominator);
    else if(point!=n)printf("ll%d %lld",signd*numerator,denominator);
}
