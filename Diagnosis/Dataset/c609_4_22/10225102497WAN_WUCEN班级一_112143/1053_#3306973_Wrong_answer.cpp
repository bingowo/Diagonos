...
int main()
{
    ...
    {
        bigint n={0,{0}};
        int specialbeginend[100][2],k=0,kk=0;
        char c = 0;
        scanf("%c", &c);
        while (c!='\n')
        {
        ...
        for (int j = 1, state = OUT;j <= n.cnt;j++)
            switch (state)
            {
            case OUT:
                if (n.v[j] == n.v[j - 1]&& n.v[j] != 9)
                {
                    state = REPETITION;
                    specialbeginend[kk++][1] = j;
                }
            case REPETITION:
                break;
            default:
        ...
        for (int j = 1,iseight=1,state=OUT;j <= n.cnt+1&&!kk;j++)
        {
            switch (state)
            {
            case OUT:
