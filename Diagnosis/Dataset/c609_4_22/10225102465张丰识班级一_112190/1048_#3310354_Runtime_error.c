...
Big dfs(Big N) {
    ...
    if (cmp(N,trans(0))==0) {
            ...
            N.co--;
           // printf ("%d\n", N.cnt);
            return f[N.co]=trans(0);
    }
    if (cmp(N,trans(1))==0||cmp(N,trans(2))==0) {
        N.co--;
        //printf ("%d\n", N.cnt);
        return f[N.co]=trans(1);
    }
    else {
   N.co--;
   return f[N.co]=add(dfs(sub(N,trans(1))),dfs(sub(N,trans(2))));
...
int main () {
  int m;
  scanf ("%d", &m);
  for (int j=0;j<125;j++) f[j]=trans(-1);
  for (int i=0;i<m;i++) {
    printf ("case #%d:\n", i);
    int t;
    scanf ("%d", &t);
