...
int main(){int num=0;char re[10000][30]={0};while(scanf("%s",&s)!=EOF){int ori=0;char a[N+1][30]={0};char c;c=getchar();c=getchar();while(c!='\n'){ungetc(c,stdin);scanf("%s",&a[ori++]);c=getchar();}
    if(ori){for(int o=0;o<ori;o++){ char b[40]={0};int min=o;
    ...
    if(j==strlen(a[min])-1||j==strlen(a[oo])-1) {if(strcmp(a[min],a[oo])>0)min=oo;}}}
    if(min!=o){strcpy(b,a[min]);strcpy(a[min],a[o]);strcpy(a[o],b);}}}
    for(int i=0;i<ori;i++){strcpy(re[num++],a[i]);if(i!=ori-1)strcpy(re[num++]," ");}strcpy(re[num++],"\n");}
    for(int kk=0;kk<num;kk++) printf("%s",re[kk]);
    return 0;
}
