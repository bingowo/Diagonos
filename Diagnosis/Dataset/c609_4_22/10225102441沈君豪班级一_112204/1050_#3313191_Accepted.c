...
int main()
{
    ...
    if(n<l){
        int shit=0;      //整无语了
        if(n<l-1){
            int cc=s1[sq1+n+2]+s2[sq2+n+2]-2*'0';
            while(cc>=10){
                shit++;
                cc-=10;
            }
        }                                    //检查两位进位 就是后两位数字一起影响四舍五入
        if(s1[sq1+n+1]+s2[sq2+n+1]+shit>'0'*2+14){
            jw=2;                               //四舍五入
        }else if(s1[sq1+n+1]+s2[sq2+n+1]+shit>='0'*2+5){
            jw=1;
        }
    }                               //进位
    for(;j>0;j--){
        if(j<=l){
           int gg=s1[sq1+j]+s2[sq2+j]-2*'0'+jw;
           jw=0;
    ...
    if(sq1<0&&sq2<0&&jw==0){         //整数部分为0 （两个数无0的情况）
        printf("0");
    }
    for(;m>=0;m--){
        if(sq1>=0&&sq2>=0){
