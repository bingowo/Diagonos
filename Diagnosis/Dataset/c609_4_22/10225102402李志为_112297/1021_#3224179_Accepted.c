    ...
    while(scanf("%s\n",xu)!=EOF){
        ...
        for(int j=0;j<strlen(p[i].s);j++){
            if(p[i].s[j]==' ' && num==0){
                ...
                t=j-n-1;
                strncpy(p[i].w[num],p[i].s+n+1,t);
                num++;
                n=j;
            }
            if(j==strlen(p[i].s)-1&&num!=0) {
                strncpy(p[i].w[num],p[i].s+n+1,strlen(p[i].s)-n);
                num++;
            }
            else if(j==strlen(p[i].s)-1){
                strncpy(p[i].w[num],p[i].s+n,strlen(p[i].s)-n);
                num++;
            }
        }//这个循环就是记下来字符串
