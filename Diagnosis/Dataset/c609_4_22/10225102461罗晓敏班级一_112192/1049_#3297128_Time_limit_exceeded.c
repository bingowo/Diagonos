...
//加法
void bigint_addition(bigint* a,bigint* b,bigint* c){           //正数a+正数b=c
	int i,temp=0;
	...
	if(i==a->cnt&&i==b->cnt&&temp==0){
		c->cnt=i;
	}
	else if(i==a->cnt&&i==b->cnt&&temp!=0){
		c->num[i]=temp;i++;
		c->cnt=i;
	}else if(i==a->cnt){
		c->num[i]=b->num[i]+temp;i++;
		while(i!=b->cnt){
			c->num[i]=b->num[i];
			i++;
		}
