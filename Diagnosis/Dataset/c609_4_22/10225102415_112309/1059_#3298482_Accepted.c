...
int cmp(const void *a, const void *b) {
	...
	if (n1 -> signal != n2 -> signal) //比较符号位 
		return n1 -> signal - n2 -> signal;
	else if (strlen(n1 -> inte) != strlen(n2 -> inte)) //若符号相同，比较整数位长度 
		if (n1 -> signal == 1) return strlen(n1 -> inte) - strlen(n2 -> inte);
		else return strlen(n2 -> inte) - strlen(n1 -> inte);
	else if (strlen(n1 -> inte) == strlen(n2 -> inte) && strcmp(n1 -> inte, n2 -> inte) != 0) { //若整数位长度相同，比较整数大小 
