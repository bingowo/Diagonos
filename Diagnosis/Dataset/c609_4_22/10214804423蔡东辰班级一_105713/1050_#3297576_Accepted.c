...
void input(bign* i, bign* f){
	...
	len2 = strlen(s2);
    int temp2 = strchr(s2, '.') - s2;
    i->len = (temp2 == 0)? 1 : temp2;
    if(temp2){
        for(int j = 0; j<i->len; j++){i->v[j] = s2[i->len - 1 - j]-'0';}
    }
    f->len = len2 - temp2 - 1;
...
int main(){
	...
	input(&ai, &af);
	input(&bi, &bf);
	int len = (af.len>bf.len)?af.len : bf.len;
	...
	bign i = add(ai, bi);
	while(i.len > 1  && i.v[i.len-1] == 0){
        i.len--;
    }
	bign f = add(af, bf);
	len = (af.len>bf.len)?af.len : bf.len;
	int N; scanf("%d", &N);
	if(N<len){
        if(f.v[len - N - 1] > 4){
            bign one = {1, {1}};
