...
{for(i=0;i<=1199;i++){R[i]=0;}
if(c[1200-P+N]>=5)
    {R[1199-P+N]=1;c[1200-P+N]=0;
for(i=1200-P+N;i>=1;i--)
{if(c[i]+R[i]>=10){R[i-1]+=(c[i]+R[i])/10;c[i]=(c[i]+R[i])%10;}
else c[i]=c[i]+R[i];}
...
void copy(int a[],char A[],int n,int v)
{
   ...
   {int j=1199-v;
     ...
     for(int i=strlen(A)-1;i>=n+1;i--)
      ...
      for(int i=n-1;i>=0;i--)
         {a[j]=A[i]-'0';j--;}}
    else{int j=1199;for(int i=strlen(A)-1;i>=0;i--){a[j]=A[i]-'0';j--;}}
//n==strlen(A)&&v==0
}
int main()
{ char A[1200],B[1200];
int i,a[1200],b[1200],N,P1,P2,k,c[1200],v=0,P;
scanf("%s %s %d",A,B,&N);
P1=strlen(A)-point(A)-1;
P2=strlen(B)-point(B)-1;
for(i=0;i<=1199;i++){b[i]=0;a[i]=0;c[i]=0;}
if(P1>=P2)
...
{while(c[v]==0){v++;}
for(i=v;i<=1199;i++){printf("%d",c[i]);}
printf(".");
for(i=0;i<N;i++){printf("0");}
}
else if(P>=N)
{while(c[v]==0){v++;}
for(i=v;i<=1199-P;i++){printf("%d",c[i]);}
if(v>1199-P)printf("0");
printf(".");
for(i=1200-P;i<=1199-P+N;i++){printf("%d",c[i]);}}
...
if(v>1199-P)printf("0");
printf(".");
for(i=1200-P;i<=1199;i++){printf("%d",c[i]);}
