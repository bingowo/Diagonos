...
{for(int p=599;p>=0;p--){A[p]=0;B[p]=0;}
    copy(stu[n].a,A);
    copy(stu[n].b,B);
if(strcmp(stu[n].a,stu[n].b)!=0){if(strlen(stu[n].a)>strlen(stu[n].b))
{p(A,B,C);}
else if(strlen(stu[n].a)==strlen(stu[n].b)&&strcmp(stu[n].a,stu[n].b)<0){p(B,A,C);printf("-");}
else if(strlen(stu[n].a)==strlen(stu[n].b)&&strcmp(stu[n].a,stu[n].b)>0){p(A,B,C);}
else{p(B,A,C);printf("-");}
int u=0;
while(C[u]==0){u++;}
