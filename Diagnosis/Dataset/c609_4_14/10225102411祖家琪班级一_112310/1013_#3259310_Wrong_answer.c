...
int main()
{
    long long a=0,b=0;
    char hex[20];
    int cplx[65],pow[2][4] = {{1,-1,0,2},{0,1,-2,2}};
    scanf("0x%s",hex);
    int l=strlen(hex),cnt2 = 0,wei = 1;
    for(int i=0;i<l;i++){
    ...
    for(int i = cnt2-1;i>-1;i--){
        int tmpa = cplx[i]*pow[0][(cnt2-i-1)%4]*wei;
        int tmpb = cplx[i]*pow[1][(cnt2-i-1)%4]*wei;
        a += tmpa;
        b += tmpb;
        cnt3++;
        if(cnt3%4 == 0) wei *= -4;
    ...
    if(a == 0){
        ...
        if(b>1 && (b<1 || b>0)) printf("%lld+%lldi",a,b);
