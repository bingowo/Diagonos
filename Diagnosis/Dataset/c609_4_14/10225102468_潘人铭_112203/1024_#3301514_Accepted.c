...
int main()
 {
    ...
    do {
        ...
        if (n != 0) {
            ...
            for (int i = 0; i < n; i++) {                scanf("%d-%d-%d %d:%d%llu",                      &infos[i].time.year, &infos[i].time.month,
&infos[i].time.day,                      &infos[i].time.hours, &infos[i].time.mins,
&infos[i].size);
            ...
            for (int i = 0; i < n; i++) {                printf("%d-%02d-%02d %02d:%02d%17llu %s\n",                       infos[i].time.year, infos[i].time.month,
infos[i].time.day,                      infos[i].time.hours, infos[i].time.mins,
infos[i].size,
                      infos[i].name);
           }
