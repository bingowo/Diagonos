...
int cmp_postive(const void* _a,const void* _b)
{
    ...
    {
        int i=0,final=0;
        for(i=0,final=0; i<strlen(a->decimal)&&i<strlen(b->decimal); i++,final++)
        {
            if(atoi(a->decimal[i])!=atoi(b->decimal[i])) return atoi(a->decimal[i])-atoi(b->decimal[i]);
        }
        if(final==strlen(a->decimal)-1||final==strlen(b->decimal)-1)
        {
            if(final=strlen(a->decimal)-1) return -1;
            else return 1;
        }
    }
...
int cmp_passive(const void* _a,const void* _b)
{
    ...
    {
        int i=0,final=0;
        for(int i=0,final=0; i<strlen(a->decimal)&&i<strlen(b->decimal); i++,final++)
        {
