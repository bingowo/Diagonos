...
{int T,i,g,z;
scanf("%d\n",&T);
char M[19][8]={"pop","no", "zip", "zotz", "tzec", "xul", "yoxkin", "mol", "chen", "yax", "zac", "ceh", "mac", "kankin", "muan", "pax", "koyab", "cumhu","uayet"};
for(i=0;i<=T-1;i++)
{
    scanf("%d\n",&g);
struct F stu[g];
for(int j=0;j<=g-1;j++)
    {fgets(stu[j].s,50,stdin);}
for(int j=0;j<=g-1;j++)
{
if(stu[j].s[1]!='.'){stu[j].day=(stu[j].s[0]-'0')*10+(stu[j].s[1]-'0');
for(int n=4;n<=strlen(stu[j].s)-2;n++){if(stu[j].s[n]==' '){ z=n;break;}}
strcpy1(stu[j].month,stu[j].s,4,z-1);
int k=1;stu[j].year=0;
for(int p=strlen(stu[j].s)-2;p>=z+1;p--){stu[j].year+=(stu[j].s[p]-'0')*k;k*=10;}
}

else if(stu[j].s[1]=='.'){stu[j].day=stu[j].s[0]-'0';
