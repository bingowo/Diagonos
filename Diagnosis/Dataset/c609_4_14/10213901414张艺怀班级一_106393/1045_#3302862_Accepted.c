...
void erase_comments(string* lines, string* ans, int cnt1)
{
    string temp;
    int state = 0;
    // 0 for global
            ...
            if (!next) {
                ...
                case '"':
                    if (temp.str[i - 1] != 92) {
                        ...
                        if (right != -1 && temp.str[right - 1] != 92) {
                            i = right;
                            state = 0;
                        }
                        else if (right != -1 && temp.str[right - 1] == 92) {
                            int cnt = 0;
...
int main()
{
    ...
    int cnt1 = 0;
    while (gets(lines[cnt1++].str) != NULL);
    cnt1--;
    string* ans = (string*)malloc(cnt1 * sizeof(string));
    erase_comments(lines, ans, cnt1);
