...
int main(){
    ...
    while(temp!=0){  //整数部分逐位+1
        integer[num1++] = (temp+1)%3;
        temp = (temp+1)/3;
    }
    for(int i=0; i<num1; i++) integer[i]--;  
    for(int i=0; i<num2; i++) decimal[i]--;
    int ret=1; //标记是否是开头的0
    if(flag==1){  //观察发现，正负的区别就是结果时候，1变成2，2变成1，0不变；
        ...
        for(int i=0; i<num1; i++){
            if(integer[num1-i-1]==0&&ret==1&&num1>1) continue; //跳过开头的0(至少保留一个)
            if(integer[num1-i-1]==-1)printf("%d", 1);
            else if(integer[num1-i-1]==1)printf("%d", 2);
            else printf("%d", 0);
            ret = 0; //遇到0不用跳
        }
    }         
    ...
    if(flag==1){  
        ...
        for(int i=0; i<num2; i++){
            if(decimal[i]==-1) printf("%d", 1);
            else if(decimal[i]==1)printf("%d", 2);
            else printf("%d", 0);
        }
    }
    free(decimal);
