...
int main(){
    long long a, b;
    scanf("%lld %lld", &a, &b);
    int *integer, *decimal, num1=0, num2=0, temp, flag;
    long long x1, x2;
    flag = 1;//标记正负
    if(a<0){
        a = a*-1;
    ...
    if(flag==-1){//处理负数的三进制(取反加一)
        for(int i=0; i<num1; i++) integer[i]=2-integer[i];
        for(int i=0; i<num2; i++) decimal[i]=2-decimal[i];
    }
    temp = (flag==1)? 0: 1;//负数二进制的加一
    ...
    if(flag==1) for(int i=0; i<num1-temp; i++) integer[i]--;  
    //经过实践证明，正数时候，进位时候多出来的一位不参与减1，而负数时候，需要参与
    else for(int i=0; i<num1; i++) integer[i]--;
    ...
    while(decimal[num2-1]==0&&num2>0) num2--; //去掉小数末尾的0
