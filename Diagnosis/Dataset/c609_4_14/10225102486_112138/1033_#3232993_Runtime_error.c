...
char *encode(char *buf,char *s){
    static const char *TransTable="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz012345689+/";
    static int j[][4]={{4,-1},{10,2,-1},{16,8,0,-1}},
                k[][5]={{6,0,-1},{12,6,0,-1},{18,12,6,0,-1}};
    int i,b=0,*p=j[strlen(s)-1];
    char *q=s;
    while(*p!=-1)
        b+=*q++<<*p++;
    for(i=0,p=k[strlen(s)-1];*p!=-1;i++,p++)
        buf[i]=TransTable[(b>>*p)&0x3f];
        while(i%4)
        buf[i++]='=';
        buf[i]=0;
    return buf;
...
void solve(){
    char s[N+1],r[(N/3+1)*3+1]=" ",t[4],buf[5];
    int i,slen;
