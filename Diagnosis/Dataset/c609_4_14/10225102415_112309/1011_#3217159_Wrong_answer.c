...
void ter2dec(char *inp, long long int inplen);
long long int threepow(long long int n);

int main(){
	char inp[50] = {0};
	long long int inplen;
	scanf("%s", inp);
	inplen = strlen(inp);
	ter2dec(inp, inplen);
	return 0;
...
long long int threepow(long long n){
	long long int ret = 1;
...
void ter2dec(char *inp, long long int inplen){
	...
	//处理小数部分
	long long int fltlen = inplen - intlen - 1;
	//printf("fltlen:%d\n", fltlen);
