...
int main(){
    ...
    for(int i=0;i<m;i++){
        ...
        scanf("%d%d%d",&c,&d,&e);
        match[i].hao=i;
        if(c==0)match[c-1].score++,match[d-1].score++,match[c-1].ping++,match[d-1].ping++;
        else if(c==1)match[c-1].score=match[c-1].score+3,match[d-1].score--,match[d-1].fail++,match[c-1].tru++;
