...
int main(){
    ...
    while (n!=0){
        ...
        scanf("%s",check);
        scanf("%s",check);
        if (strcmp(check,"/NAME")==0) qsort(file+1,n,sizeof(file[1]),cmp1);
        if (strcmp(check,"/SIZE")==0) qsort(file+1,n,sizeof(file[1]),cmp2);
        if (strcmp(check,"/TIME")==0) qsort(file+1,n,sizeof(file[1]),cmp3);
        for (int i=1;i<=n;i++){
