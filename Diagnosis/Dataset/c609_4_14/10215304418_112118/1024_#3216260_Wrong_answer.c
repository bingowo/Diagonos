...
int cmp2(const void* datai,const void* dataj){
    struct filename *dai=(struct filename *)datai;
    struct filename *daj=(struct filename *)dataj;
    return dai->size-daj->size;
}

int cmp3(const void* datai,const void* dataj){
    struct filename *dai=(struct filename *)datai;
    struct filename *daj=(struct filename *)dataj;
    if (strcmp(dai->st1,daj->st1)==0) return strcmp(dai->st2,daj->st2);
...
int main(){
    ...
    while (n!=0){
        ...
        char check[1234];
        gets(check);
        if (check=="LIST /NAME") qsort(file+1,n,sizeof(file[1]),cmp1);
        if (check=="LIST /SIZE") qsort(file+1,n,sizeof(file[1]),cmp2);
        if (check=="LIST /TIME") qsort(file+1,n,sizeof(file[1]),cmp3);
        for (int i=1;i<=n;i++){
            printf("%s %s %d %s\n",file[i].st1,file[i].st2,file[i].size,file[i].st3);
        }
        scanf("%d",&n);
