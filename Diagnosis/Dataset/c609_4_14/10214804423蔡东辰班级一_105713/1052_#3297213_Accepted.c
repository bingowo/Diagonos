...
int main(){
	bign L[25] = {{1,{2}}, {1, {3}}}; int cnt = 2;
	int table[1000] = {0,0,0,1};
	for(int i = 5; cnt<25; i++){
		int pri = 1;
		...
		if(pri){table[i] = cnt; i2init(i, &L[cnt++]);}
	}
	int temp[25]; int cntm = 0; char c;
	while(1){
		scanf("%d", &temp[cntm++]); 
		c = getchar();
		if(c=='\n') break;
	...
	bign array[25] = {{1, {1}}};
	for(int i = 1; i<25; i++){
		array[i] = Multi(array[i-1], L[i-1]);
//		print(&array[i]);
//		printf("\n");
	}
	bign r = {0, {0}};
	for(int i = cntm-1, j = 0; i>=0; i--, j++){
		bign c = multi(array[j], temp[i]);
		r = add(c, r);
	}
	while(r.len > 1  && r.v[r.len-1] == 0){
        r.len--;
    }
	print(&r);
	return 0;
