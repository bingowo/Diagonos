...
int main() {
    ...
    for(i=dot1-1,j=dot2-1;i>=0||j>=0;i--,j--){//整数部分相加，ans在front里
       if(i>=0&&j>=0){//整数部分都未遍历完
       front[--max]=front1[i]+front2[j]+carry;
       if(front[max]>9){front[max]%=10;carry=1;}
       else carry=0;}
       else if(i<0&&j>=0){flag=1;//整数部分不同时遍历完
        while(j>=0){
            front[--max]=front2[j--]+carry;
            if(front[max]>9){front[max]%=10;carry=1;if(j<0)front[--max]=1;}//进位
            else carry=0;
        }
       }
       else if(i>=0&&j<0){flag=1;//整数部分不同时遍历完
        while(i>=0){
           ...
           if(front[max]>9){front[max]%=10;carry=1;if(i<0)front[--max]=1;}//进位
           else carry=0;
        }
       }
    }
    if(flag==0){//整数部分同时遍历完
        if(carry==1)front[--max]=1;//进位
       }
    // 根据小数部分精度进行四舍五入，输出结果
    int flag1=0,flag2=0;
    if(len>=maxb){
        for(i=max;i<MAX_N;i++){
        if(front[i]){flag1=1;}    
        if(flag1){printf("%d",front[i]);flag2=1;}
        }
        if(!flag2){printf("0");}
        printf(".");
        for(i=0;i<maxb;i++){
