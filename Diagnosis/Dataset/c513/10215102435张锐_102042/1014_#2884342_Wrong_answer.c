...
void ito3(long long int n,char *s)
{
    long long int a=abs(n),i=0;
    int b=3;
    while(a!=0)
    {
        s[i]=a%b+'0';
...
void add(char *s,int num)
{
    ...
    if(num>2)
    {
        ...
        len=strlen(s);
        for(int i=len-num,j=0;i>=0&&j<6;i--,j++)
        {
            int x=(s[i]-'0')+1+last;
            if(x==3) {s[i]='0';last=1;}
...
{
    int len;
    if(num>2) len=strlen(s)-1;
    if(num==2) len=strlen(s);
    for(int i=len-num+1,j=0;i>=0&&j<6;i--,j++)
    {
        int x=(s[i]-'0')-1;
        if(x!=(-1)) s[i]=x+'0';
...
int main()
{
    ...
    minus(s,num);
    int i;
    if(num>2) i=strlen(s)-1;
    else if(num==2) i=strlen(s);
    int point=i-num;
    for(;i>point;i--)
    {
        if(s[i-1]!='0') break;
    ...
    for(int j=0;j<i;j++)
    {
        if((s[j]!='0'||k)&&j!=point) {printf("%c",s[j]),condition=1,k=1;}
        else if(j==point&&condition) {printf(".");j--;point=-1;k=1;}
        else if(j==point&&(~condition)) {printf("0.");point=-1;j--;k=1;}
    }
    return 0;
}
