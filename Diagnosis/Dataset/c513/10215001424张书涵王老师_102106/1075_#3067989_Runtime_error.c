...
void calculate_path(struct elem a[][100],int M,int N)
{
    int i,j;
    for(i=1;i<M;i++)
    {
...
void move_path(int a[][100],int r,int c,char* s,int k)
{
    if(r==0&&c==0)
        return ;
    else
    {
        ...
        {
            s[k]='R';
            move_path(a,r,c-1,s,k+1);
        }
        else
        {
            s[k]='D';
            move_path(a,r-1,c,s,k+1);
        }
    }
}
