...
long long tonum(const char*s, int begin, int end){
    for(int i=begin;i<=end;i++)cbuf[cbufsp++]=s[i];
    cbuf[cbufsp]=0;
    cbufsp=0;
    return atoll(cbuf);
}
    ...
    while(p>=0){
        while(flag+tag<3 && p>=0 && order[p]!='('){
            if(order[p]==' ')p--;
                ...
                if(flag==1) lval=*(solve(order,p));
                else if(flag==0) rval=*(solve(order,p));
                p=buf[--bufsp];
                flag++;
            }
...
int main()
{
    ...
    for(int i=0;i<variasp;i++)free(varia[i]);
    return 0;
}
