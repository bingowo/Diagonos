...
void init (char *s,bdou* n){
    ...
    if(p==-1){
        ...
        for(int i=strlen(s)-1;i>=0;i--)
            n->bin[n->bp++]=s[i]-'0';
        while(n->bp>1 && n->bin[n->bp-1]==0)n->bp--;
    }
    else if(p==0){
        n->bp=1;n->bin[0]=0;
...
int cmp(bdou* n1,bdou* n2){
    if(n1->bp!=n2->bp){
        return n1->bp-n2->bp;
    }
    else{
        for(int i=n1->bp-1;i>=0;i--){
            if(n1->bin[i]!=n2->bin[i])return n1->bin[i]-n2->bin[i];
        }
        for(int i=0;i<n1->np || i<n2->np;i++){
            if(n1->nim[i]!=n2->nim[i])return n1->nim[i]-n2->nim[i];
        }
        return 0;
    }
}
