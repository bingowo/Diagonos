...
void full(num* map,num* s,int p){
    ...
    for(;rr<toppos && !(s->n==0 && s->fz==0);rr++){
        if(isequal(newmap[rr],newmap[0]));
            ...
            if((1.0*out.fz/out.fm)*rr<=(1.0*s->fz/s->fm)) {
                ...
                *s=sub(*s,out);
                ...
                s->fm*=rr;newmap[0]=add(newmap[0],*s);
                s->fz=0;s->n=0;break;
            }
        }
    }
    ...
    /*
    if(isequal(newmap[0],newmap[toppos])){
        if(isequal(map[lpos],map[rpos])) map[lpos].index=map[rpos].index=0;
        else{
            int lowerpos=cmp(&map[lpos],&map[rpos])==-1?lpos:rpos;
            map[lowerpos].index=0;
        }
    }*/
