...
num search(num* valley,int pos,int dir,long long* bound){//-1:left +1:right
    num ret = {0,0,1};
...
num solve(num input,num* valley,long long pos){
...
int main(){
    ...
    num valley[n+2];
    valley[0].n = valley[0].fz = valley[n+1].n = valley[n+1].fz = 2000;
    for(int i = 1;i < n+1;i++){
