...
int cmp(const void*a,const void*b){
    ...
    else if(s1.fz*s2.fm!=s2.fz*s1.fm) return s1.fz*s2.fm<s2.fz*s1.fm?-1:1;
    else return 0;
}
...
void full(num* map,num* s,int p,int dir){
    ...
    for(int i=rpos-lpos+1;i>=0;i--){
        if(cmp(&newmap[i-1],&map[rpos])==0 || cmp(&newmap[i-1],&map[lpos])==0);
        else {toppos=i;break;}
    }

    int rr=0;
    for(;rr<toppos && !(s->n==0 && s->fz==0);rr++){
        if(cmp(&newmap[rr],&newmap[0])==0);
        else{
    ...
    if(!(s->n==0 && s->fz==0) && cmp(&newmap[rr],&newmap[0])!=0){
        num out=sub(newmap[rr],newmap[0]);
        out.fz*=rr;out.n*=rr;simplify(&out);
        if(cmp(&out,s)<=0){
            int tmp=newmap[0].index;
...
void solve(num* map,num* s,int p){
    ...
    if(cmp(&map[topl],&map[p])!=0){
        ...
        while(!(halfs1.n==0 && halfs1.fz==0) && p!=halfs1.index){
            solve(map,&halfs1,halfs1.index);
        }
        while(!(halfs2.n==0 && halfs2.fz==0) && p!=halfs2.index){
