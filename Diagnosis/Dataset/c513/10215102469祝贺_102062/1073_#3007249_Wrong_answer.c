...
long long dfs(int* num, int* n, int (*dp)[9], int sum, int depth, int limit){
    long long ret = 0;
    if(depth >= n[19]) return ((sum)%9 == 0?0:1);
    else{
        int k = limit == 1?n[depth]+1:9;
        for(int i = 0; i < k; i++){
            if(!(i == k-1 && limit == 1) && dp[depth+1][8] != -1){
                for(int j = 0; j < 9; j++){
                    if((sum+i+j)%9!=0) ret += dp[depth+1][j];
                    if((j+i)%9 != 0) dp[depth][i] += dp[depth + 1][j];
                }
            }
            else {
