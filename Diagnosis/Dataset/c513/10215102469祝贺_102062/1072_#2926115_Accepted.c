	...
	if(x==0 && y==0)p=start;
		...
		while(rear!=front){
			...
			if(p.x==0 && p.y==0) break;
				...
				node q1=p;
				if(p.x<p.y){q1.x+=step;q1.path[q1.sp++]='W';}
				else{q1.y+=step;q1.path[q1.sp++]='S';}
				q[rear++]=q1;
			}
			else if(p.x<=0 && p.y>=0){
				node q1=p;
				if(-p.x<p.y){q1.y-=step;q1.path[q1.sp++]='N';}
				else{q1.x+=step;q1.path[q1.sp++]='W';}
				q[rear++]=q1;
			}
			else if(p.x>=0 && p.y<=0){
				node q1=p;
				if(p.x<-p.y){q1.y+=step;q1.path[q1.sp++]='S';}
				else{q1.x-=step;q1.path[q1.sp++]='E';}
				q[rear++]=q1;
			}
			else if(p.x>=0 && p.y>=0){
				node q1=p;
