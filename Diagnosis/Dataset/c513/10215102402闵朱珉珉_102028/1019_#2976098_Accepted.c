...
int main()
{
    while (1)
    {
        ...
        for (int i=0;i<m;i++)
        {
            ...
            switch (c)
            {
                case 1: s[a-1].score+=3;s[a-1].win+=1;s[b-1].score-=1;s[b-1].defeat+=1;break;
                case -1: s[a-1].score-=1;s[a-1].defeat+=1;s[b-1].score+=3;s[b-1].win+=1;break;
                case 0: s[a-1].score+=1;s[b-1].score+=1;break;
            }
        }
