...
int main()
{
    ...
    for(i = 0; i < n; i++)
    {
       ...
       for(j = 0; j < strlen(s); j++)//枚举每一个字母后面插入和他相同的字母
       {
           ...
           {
               strcpy(s2,s1);
               for(l = 0; l < k; l++)s1[l] = '\0';//首字母单独讨论
               for(l = 0,m = 0; l < strlen(s2)-1; l++)
               {
                   if(l == 0){if(s2[0] != s2[1])s1[m] = s2[l],m++;}//如果这个字母与前或后相同则不记录，不同就记录；
                   else if((s2[l] != s2[l-1])&&(s2[l] != s2[l+1]))s1[m] = s2[l],m++;
               }
               if(s2[l] != s2[l-1])s1[m] = s2[l],m++;//末字母也单独讨论
               s1[m] = '\0';//结束符结束符！！
