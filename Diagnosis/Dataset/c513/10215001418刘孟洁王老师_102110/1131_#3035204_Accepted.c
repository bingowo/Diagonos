...
int main()
{
    ...
    if(strlen(zsum+j)&&strlen(xsum+i))printf("%s.%s",zsum+j,xsum+i);
    else if(!strlen(zsum+j))
    {
        if(!strlen(xsum))printf("0");
        else printf("0.%s",xsum+i);
    }
    else if(!strlen(xsum+i))printf("%s",zsum+j);
    return 0;
}