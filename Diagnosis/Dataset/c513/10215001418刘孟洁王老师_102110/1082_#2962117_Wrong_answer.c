...
void read1(char s[])
{
    ...
    if(i == 0)
       ...
       else if(i == len-1){for(j = 0; j < len-1; j++)s1[j] = s[j];s1[j] = '\0';s2[0] = '0';s2[1] = '\0';}
       else
       {
...
void read2(char s[])
{
    ...
    for(i = 0;i < len; i++)
       ...
       else if(i == len-1){for(j = 0; j < len-1; j++)ss1[j] = s[j];ss1[j] = '\0';ss2[0] = '0';ss2[1] = '\0';}
       else
       {
           for(j = 0;j < i; j++)ss1[j] = s[j];
           ss1[j] = '\0';
...
int main()
{
    ...
    char fl1[N],fl2[N],s[N];
        ...
        for(i = 0; i < l; i++)
        {
          if(sumz[i] == '0'&& l != 1)continue;
          else if(sumz[i] != '0')break;
        }
        for(k = 0;i < l; k++,i++)s[k] = sumz[i];
        s[k] = '\0';
        sumx[num] = '\0';
        //printf("%s %s %s %s\n",s1,s2,ss1,ss2);
        printf("%s.%s\n",s,sumx);
