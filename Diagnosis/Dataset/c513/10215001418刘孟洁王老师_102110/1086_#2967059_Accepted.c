...
void minus_float(char s1[],char s2[])
{
    ...
    if(len1 < num&&len2 < num)len = num;//若小数不够长；
 ...
 void minus_int(char s1[],char s2[])
 {
    ...
    if(po-m == 1)ans1[0] -= 1;//判断进位和借位是否抵消！！
    for(i = 0; i < len-1; i++)
    {
        if(ans1[i] < '0')
        {
...
int main()
{
    ...
    char si1[1007],sf1[1007],si2[1007],sf2[1007],anss[1007];
    int len,len1,len2,l,k = 0,i;
    ...
    for(i = 0,l = 0; i <len; i++)
    {
        if(f && ans1[i] == '0')continue;
        f = 0;
        anss[l] = ans1[i];
        l++;
    }
    anss[l] = '\0';
    if(!strlen(anss)){anss[0] = '0';anss[1] = '\0';}
    ans2[num] = '\0';
    if(k == 1)printf("-%s.%s\n",anss,ans2);
