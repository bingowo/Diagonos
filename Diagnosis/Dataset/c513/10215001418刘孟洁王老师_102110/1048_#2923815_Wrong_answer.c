    ...
    int i,j,k,l,h,g,x;
        while(scanf("%s%s",s1,s2)!=EOF)
       {
        ...
        for(i = 0; i < strlen(s1); i++)
        {
            ...
            else if(isdigit(s1[i])&&s1[i+1] == '\0'&&s1[i-1] != '^')
            {
                if(s1[i-1] == '-')t1[k].xi = -1*(s1[i]-'0');
                else t1[k].xi = s1[i]-'0';
                t1[k].ci = 0;
        ...
        for(i = 0; i < strlen(s2); i++)
        {
            ...
            else if(isdigit(s2[i])&&s2[i+2] == 'x')
            {
                ...
                {
                    t2[l].ci = s2[i+3]-'0';
                    if(isdigit(s2[i+4]))t1[l].ci = t2[l].ci*10 + s2[i+4]-'0';
                    }
                else t1[l].ci = 1;
                l++;
            }
            ...
            for(i = 0,x = 0; i < h; i++)
             {
                ...
                if(sum[i].ci >= 0&&sum[i].xi != 0)sum1[x] = sum[i],x++;
             }
