...
int main()
{
   ...
   for(i = 3; i <= n; i++)
   {
    ...
    px[1].aa = 0;px[1].tt = 0;
   px[2].aa = 0;px[2].tt = 1;//下车系数（其实下车也满足斐波那契数列）
   for(i = 3; i <= n; i++)px[i] = ps[i-1];
   sum[1].aa = 1;sum[1].tt = 0;
   sum[2].aa = 1;sum[2].tt = 0;//总人数系数
   for(i = 3; i <= n; i++)
   {
       sum[i].aa = sum[i-1].aa + ps[i].aa - px[i].aa;//上一次的系数加上上车的，减去下车的
       sum[i].tt = sum[i-1].tt + ps[i].tt - px[i].tt;
    }
   t = (m-sum[n-1].aa*a)/sum[n-1].tt;//利用最后的总人数和系数求出t的值（第二站上车的人数）
