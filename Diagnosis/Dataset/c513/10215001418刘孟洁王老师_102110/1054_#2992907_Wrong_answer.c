...
#include <stdlib.h>
#include <string.h>
char ans[107][77],ss[107][37];
int main()
{
    int n,i,cas,j,r,k,l,p,g,m,h,b,b1,num,len[1007];
    char s[2007];
    i = 0;
    scanf("%d",&n);
    gets(s);
       ...
       for(i = 0,l = 0,p = 0,k = 0; i < cas; i++)
       {
          ...
          else
         {
            ...
            for(j = 0,m = 0,g = i-k+1; j < k-1; g++,j++)
            {
                ...
                for(h = 0; h < b1&&m < num;m++,h++)ans[p][m] = ' ';
