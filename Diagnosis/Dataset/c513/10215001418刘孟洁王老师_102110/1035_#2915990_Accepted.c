...
int main()
{
    long long int n,m,i,j,SSS1,SSS2;
    column *s,*ss,*t;
    scanf("%lld%lld",&n,&m);
    ...
    SSS1 = s[0].S;
    for(j = 0; j < m-1; j++)SSS1 = SSS1 + ss[j].S;//算第二个到m个这些的侧面积和；
    SSS1 = SSS1 + s[0].SS;//加上最大的底面积；
    ...
    SSS2 = s[0].S;
    for(j = 0; j < m-1; j++)SSS2 = SSS2 + ss[j].S;//算第二个到m个这些的侧面积和；
    qsort(t,m,sizeof(t[0]),cmp2);//再按半径大小降序排列；
    SSS2 = SSS2 + t[0].r * t[0].r;//加上半径最大的圆的面积；
    printf("%lld\n",max(SSS1,SSS2));
    return 0;
