...
int pourWater(int s, int p) {
    int lv=0, rv=0, lp=p, rp=p, v;
    for(int i=p-1; i>=0; --i) {
        lp = i;
        if(a[i].v > a[p].v) break;
        lv += a[p].v - a[i].v;
    }
    ...
    v = lv + rv;
    if(v == 0) {
        return lowestWater(s,a[p].v, lp, rp);
    }else if(v <= s) {
        ...
        Number half;
        half.v = s/2; half.fz = s%2; half.fm = 1+s%2;
        if(half.v >= lv) {
            for(int i=lv+1; i<p; ++i) a[i].v = a[p].v;
            half.v = s - lv; half.fz = 0; half.fm = 1;
            full(half, p, rp, 1);
        }else if(half.v >= rv) {
            for(int i=p+1; i<rp; ++i) a[i].v = a[p].v;
...
Number fullSub(Number s, int lp, int rp, int dir) {
    ...
    for(int i=lp+dir; i!=tp; i+=dir) //question ???
        v += lim - a[i].v;
    if(s.v >= v) {
        for(int i=lp+dir; i!=tp; i+=dir) a[i].v = lim;
        s.v -= v;
        return s;
    }else {
...
void fullFinal(Number s, int lp, int rp, int dir) {
    int arr[1002], t = 0;
    for(int i=lp; i!=rp; i+=dir) arr[t++] = a[i].v;
