...
char* plus(char s[],char c[],char s1[],char c1[],int N){
	int i,j,t=1;
...
int main(){
	...
	char s1[max],c1[max];
	int i;
	for(i=0;i<max;i++) s1[i]='0',c1[i]='0';
	s1[i]='\0',c1[i]='\0';
	int N;
	scanf("%s %s %d",s,c,&N);
	if(pd(s,c)>=0) printf("%s",plus(s,c,s1,c1,N));
	else printf("%s",plus(c,s,s1,c1,N));
	return 0;
}