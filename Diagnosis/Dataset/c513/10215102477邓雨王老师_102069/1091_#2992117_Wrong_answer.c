...
void print(pair*t){
	int i,p,j;
	for(i=3999;t->a[i]==0;i--);  //确定不为0的位置
}
int main(){
	char s[100];
	int chance,i,key=0;
	scanf("%s %d",&s,&chance);
	if(chance==0) {printf("1");return 0;}
	pair x,y;
	x.sign=1,y.sign=1;
	for(i=0;i<4000;i++) x.a[i]=0;
	for(i=0;i<4000;i++) y.a[i]=0;
	read(s);
	pair *x1,*y1;
	x1=&x,y1=&y;  
	convert(a,x1);
    ...
    if(pd(x1)==-1) {}
    if(x1->sign==-1) printf("-");
    for(i=pd(x1);i>=0;i--) {printf("%d",x1->a[i]);key=1;}
    if(pd(y1)!=-1){
    if(y1->sign==-1) printf("-");
    else printf("+");
