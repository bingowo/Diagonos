    #include<stdio.h>
    #include<stdlib.h>
    #include<string.h>

    int main()
    {
        char A[200]={'\0'},B[200]={'\0'};
        scanf("%s %s",A,B);
        int flagA=1,flagB=1;
        if(A[0]=='-') {for(int i=0;i<strlen(A);i++) A[i]=A[i+1];flagA=-1;}
        if(B[0]=='-') {for(int i=0;i<strlen(B);i++) B[i]=B[i+1];flagB=-1;}
        if(strcmp(A,B)==-1 && flagA==1 && flagB==-1) printf("-");
        if(strcmp(A,B)==1 && flagA==-1 && flagB==1) printf("-");
        if(flagA==-1 && flagB==-1) printf("-");
        if(strcmp(A,B)==-1) {char tmp[200];strcpy(tmp,A);strcpy(A,B);strcpy(B,tmp);}
        char tmpA[200]={'\0'};strcpy(tmpA,A);for(int i=0;i<strlen(tmpA);i++) A[strlen(tmpA)-1-i]=tmpA[i];
        char tmpB[200]={'\0'};strcpy(tmpB,B);for(int i=0;i<strlen(tmpB);i++) B[strlen(tmpB)-1-i]=tmpB[i];
        for(int i=strlen(B);i<strlen(A);i++) B[i]='0';
        char res[200]={'\0'};
        int c=0,cnt=0,i=0;
        if(flagA*flagB>0)
            for(int i=0;i<strlen(A);i++)
            {
                res[i]=(A[i]-'0'+B[i]-'0'+c)%10+'0';
                c=(A[i]-'0'+B[i]-'0'+c)/10;
            }
        else
            for(int i=0;i<strlen(A);i++)
            {
                if(A[i]<B[i]) {res[i]=10+A[i]-'0'-(B[i]-'0')+'0';A[i+1]--;}
                else res[i]=A[i]-'0'-(B[i]-'0')+'0';
            }
        i=strlen(res)-1;
        for(;i>=0;i--) if(res[i]!='0') break;
        for(;i>=0;i--) printf("%c",res[i]);
        return 0;
    }