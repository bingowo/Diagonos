...
//    printf("%d\n",(int)(B->product-A->product));
    return (int)(B->product-A->product);
}
long long  cs(long long  ans,long long  cp){
    if(ans>= cp){
        return ans;
    }else{
        return cp;
...
int main(){
    ...
    for(int i = 0; i < n;i++){
        ...
        a[i].product = 1LL*a[i].height*a[i].radius;
    }
    cylinder ax[1001];
    for (int i = 0;i < n;i++){
        int k = 0;
        ...
        if(k!=0 && k>=m-1){
            ...
            ans = cs(ans,area*2+a[i].radius*a[i].radius+2*a[i].product);
        }
    }
    printf("%lld",ans);
