...
int main()
{
    char tmp[20]={'\0'};
    scanf("%s",tmp);
    char s[100]={'\0'};strcpy(s,transBin(tmp));
    int l=strlen(s);
    int real=0,vir=0;
    for(int i=0;i<l;++i){
        if(s[i]=='1'){
    ...
    // printf("%d,%d",real,vir);
    if(vir==0)printf("%d",real);
    else if(real==0)printf("%di",vir);
    else if(vir<0)printf("%d-%di",real,-vir);
    else printf("%d+%di",real,vir);
    return 0;
}
