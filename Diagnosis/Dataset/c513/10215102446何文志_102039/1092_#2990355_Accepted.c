...
int main()
{
    ...
    for(int i = 0;i<strlen(w);i++){
        char* curPossible = (char*)malloc(100*sizeof(char));
        memset(curPossible,0,100*sizeof(char));
        curPossible[0] = '0';
        for(int j = i-1;j<=i+1;j++){
            ...
            if(flag == 1){
                char* tmp = BigNumAdd(curPossible,"1");
                free(curPossible);
                curPossible = tmp;
            }
        }
        ...
        char* tmp1 = BigNumMod(res,MOD);
        //char* tmp2 = BigNumMod(curPossible,MOD);
        char* tmp3 = BigNumMultiply(curPossible,res);
        free(res);
        res = tmp3;
        free(tmp1);
        //printf("res = %lld\n",res);
