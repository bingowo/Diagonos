...
int cmp(const void* a, const void* b){
    return BigNumCmp((char*)a, (char*)b);
}
...
int main()
{
    int n = 0;
    scanf("%d",&n);
    char s[n][150];memset(s,0,n*150*sizeof(char));

    for(int i = 0;i<n;i++){
        //s[i] = (char*)malloc(150*sizeof(char));
        //memset(s[i],0,150*sizeof(char));
        scanf("%s",s[i]);
    }
