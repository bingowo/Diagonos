...
int main()
{
    ...
    //printf("fac = %s\n",frac);
    if(strlen(whole)==0){
        whole = (char*)malloc(10*sizeof(char));
        memset(whole,0,10*sizeof(char));
        whole[0] = '0';
        whole[1] = 0;
    }
    if(strlen(frac)==0){
        frac = (char*)malloc(10*sizeof(char));
        memset(frac,0,10*sizeof(char));
        frac[0] = '0';
        frac[1] = 0;
    }
    while(strlen(frac)<N){
    ...
    while(whole[0]=='0'){
        for(int k = 0;k<strlen(whole)-1;k++){
            whole[k] = whole[k+1];
        }
