...
int main()
{
    ...
    for(int i = 0;i<T;i++){
        ...
        char* resBin = (char*)malloc(500*sizeof(char));
        memset(resBin,0,500*sizeof(char));
        int resCnt = 0;
        //printf("hhs = %s\n",s);
        //printf("h %d\n",BigNumCmp(s,"0"));
        while(BigNumCmp(s,"0")>0){
            char* r = BigNumMod(s,"2");
            resBin[resCnt++] = r[0];
            char* tmp = BigNumDivide(s,"2");
            free(s);
            s = tmp;
            //printf("r = %s,s = %s\n",r,s);
        ...
        char* res = (char*)malloc(110*sizeof(char));
        memset(res,0,110*sizeof(char));
        res[0] = '0';
        for(int j = 0;j<strlen(resBin);j++){
            char* curNum = (char*)malloc(2*sizeof(char));
            curNum[0] = resBin[j]; curNum[1] = 0;
            char* tmp = BigNumMultiply(res,"2");
            char* tmp1 = BigNumAdd(tmp,curNum);
            free(res);free(tmp);
            res = tmp1;
            //printf("res = %s\n",res);
        }
