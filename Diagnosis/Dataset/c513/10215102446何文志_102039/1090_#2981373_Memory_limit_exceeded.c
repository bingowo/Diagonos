...
int main()
{
    ...
    while(1){
        ...
        res[0] = '1';
        char N1[10];memset(N1,0,10*sizeof(char));
        sprintf(N1,"%d",N);

        char* dp[10];memset(dp,0,10*sizeof(char));
        dp[0] = N1;
        for(int i = 1;i<10;i++){
            dp[i] = BigNumMultiply(dp[i-1],dp[i-1]);
        }
        for(int i = 0;i<10;i++){
            //printf("%d: %s\n",i,dp[i]);
        }
        ...
        int cnt = N;
        while(cnt>0){
            for(int i = 29;i>=0;i--){//dp[i]
                int curDP = 1<<i;
                if(cnt>=curDP){
                    res = BigNumMultiply(res,dp[i]);
                    cnt-=curDP;
                    //printf("res = %s, cnt = %d, curDp = %d\n",res,cnt,curDP);
