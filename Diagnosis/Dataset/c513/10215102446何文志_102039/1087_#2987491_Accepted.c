    ...
    for(int i = 0;i<T;i++){
        ...
        while(1){
            ...
            if(flag == 1){//满足条件，退出
                ...
                char* add = (char*)malloc((digit+10)*sizeof(char));
                memset(add,0,(digit+10)*sizeof(char));
                add[0] = '1';
                for(int k = 1;k<digit;k++){
                    add[k] = '0';
                }
                //printf("add = %s\n",add);
                char* tmp = BigNumAdd(add,s);
                free(s);
                s = tmp;
            }
            //重置最小置为0101...
            for(int k = j+1;k<strlen(s);k++){
                if(((k-j)%2)==1){//后面第x个数，x为奇数，则为0，x为偶数，则为1
                    s[k] = '0';
                }
                else{
                    s[k] = '1';
