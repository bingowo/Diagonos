...
{
    int* arr = malloc(arrlen*sizeof(int));memset(arr,0,arrlen*sizeof(int));
    int n = 0,temp = 1;
...
int main()
{
    ...
    for(int i = 0;i<10;i++){printf("%d",tempArr[i]);}
    free(tempArr);
    //
    int j = 0;
    while(j<digits)
    {
        ...
        char sclip[4];memset(sclip,0,4);//防止strlen连续计算sclip后面多加一个0
        for(int i = 0;i<3;i++){sclip[i] = s[j++];}
