...
int main()
{
    ...
    while(1){
        ...
        if(strcmp(curOperation,"IN")==0){
            ...
            int tmp = *p;
            setRegister(reg1, r, tmp);
        }
        else if(strcmp(curOperation,"XCHG")==0){
            char reg1[3] = {0,0,0}, reg2[3] = {0,0,0};
            scanf("%[A-Z],%[A-Z]",reg1,reg2);
            int *p1 = getRegister(reg1, r), *p2 = getRegister(reg2, r);//寄存器的地址
            int tmp1 = *p1, tmp2 = *p2;//临时存放
            setRegister(reg1, r, tmp2);
            setRegister(reg2, r, tmp1);
        }
            ...
            if(strlen(reg3)==0){//未读入reg3
                ...
                else if(strcmp(curOperation,"DIV")==0){*p1 = (int)((*p1) / (*p2));}
                ...
                else if(strcmp(curOperation,"DIV")==0){*p1 = (int)((*p2) / (*p3));}
