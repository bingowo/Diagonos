...
#include <stdlib.h>
#include<string.h>
typedef struct{char a[500];int year;int month;int day;int hour;int minute;long long size;char name[200];} file;
...
int main()
{
    ...
    while(scanf("%d",&n)!=EOF&&n!=0)
    {
      ...
      gets(order);
      if(!strcmp(order,"LIST /NAME\0")){qsort(f,n,sizeof(f[0]),cmp1);}
      if(!strcmp(order,"LIST /SIZE\0")){qsort(f,n,sizeof(f[0]),cmp2);}
      if(!strcmp(order,"LIST /TIME\0")){qsort(f,n,sizeof(f[0]),cmp3);}
