...
{if(a[i]>=b[i]){a[i]=a[i]-b[i]+'0';}
   ...
   a[i+x]--;
   a[i]=a[i]-b[i]+'0'+10;}}
   for(i=strlen(a)-1;a[i]=='0';i--) a[i]=0;
   strcpy(c2,a);
}
void imultiply(char*b1,char*b2,int p1,int p2)
{char d1[200]={0},d2[200]={0},d3[200]={0},d4[200]={0};
multiply(b1,c1,d1);
multiply(b2,c2,d2);
multiply(b1,c2,d3);
multiply(b2,c1,d4);
if(p1*q1>0&&q2*p2<0) {plus(d1,d2);q1=1;}
if(p1*q1<0&&q2*p2>0) {plus(d1,d2);q1=-1;}
if(p1*q1>0&&p2*q2>0){if(compare(d1,d2)) {q1=1;minus(d1,d2);}
                     else {q1=-1;minus(d2,d1);}}
if(p1*q1<0&&p2*q2<0){if(compare(d2,d1)) {q1=1;minus(d2,d1);}
                     else {q1=-1;minus(d1,d2);}}
if(p1*q2>0&&q1*p2>0) {plus(d3,d4);q2=1;}
if(p1*q2<0&&q1*p2<0) {plus(d3,d4);q2=-1;}
if(p1*q2>0&&q1*p2<0){if(compare(d3,d4)) {q2=1;minus(d3,d4);}
                     else {q2=-1;minus(d4,d3);}}
if(p1*q2<0&&q1*p2>0){if(compare(d4,d3)) {q2=1;minus(d4,d3);}
                     else {q2=-1;minus(d3,d4);}}

}
int main()
{char a[20],b1[200]={0},b2[200]={0},b3[200];
b1[0]='0';
b2[0]='0';
int n,p1,p2,i=0,x,k;
scanf("%s%d",a,&n);
