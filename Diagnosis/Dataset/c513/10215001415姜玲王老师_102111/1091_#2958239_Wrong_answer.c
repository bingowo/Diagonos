...
multiply(b1,c2,d3);
multiply(b2,c1,d4);
if(p1*q1>0&&q2*p2<0) {plus(d1,d2,c1);q1=1;}
if(p1*q1<0&&q2*p2>0) {plus(d1,d2,c1);q1=-1;}
if(p1*q1>0&&p2*q2>0){if(compare(d1,d2)) {q1=1;minus(d1,d2,c1);}
                     else {q1=-1;minus(d2,d1,c1);}}
if(p1*q1<0&&p2*q2<0){if(compare(d2,d1)) {q1=1;minus(d2,d1,c1);}
                     else {q1=-1;minus(d1,d2,c1);}}
if(p1*q2>0&&q1*p2>0) {plus(d3,d4,c2);q2=1;}
if(p1*q2<0&&q1*p2<0) {plus(d3,d4,c2);q2=-1;}
if(p1*q2>0&&q1*p2<0){if(compare(d3,d4)) {q2=1;minus(d3,d4,c2);}
                     else {q2=-1;minus(d4,d3,c2);}}
if(p1*q2<0&&q1*p2>0){if(compare(d4,d3)) {q2=1;minus(d4,d3,c2);}
                     else {q2=-1;minus(d3,d4,c2);}}
