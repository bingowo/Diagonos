        ...
        while (i>=0)
        {
            ...
            else if(c=='+'||c=='-'||c=='*')
            {
                ...
                if(isdigit(s[0]))
                {
                    switch (flag)
                    {
                        case '+':res.num[0]=tmp+atoll(s);break;
                        case '-':res.num[0]=tmp-atoll(s);break;
                        case '*':res.num[0]=tmp*atoll(s);break;
                        default:break;
                    }
            ...
            else if(c=='/')
            {
                ...
                tmpidx=searchmap(s);
                for(int j=1;j<map[tmpidx].vectorlen;j++)
                {
                    switch (flag)
                    {
                        case '+':map[tmpidx].num[0]+=map[tmpidx].num[j];break;
                        case '-':map[tmpidx].num[0]-=map[tmpidx].num[j];break;
                        case '*':map[tmpidx].num[0]*=map[tmpidx].num[j];break;
                        default:break;
                    }
