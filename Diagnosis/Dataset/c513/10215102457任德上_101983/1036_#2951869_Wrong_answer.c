...
unsigned long long int cal_dis1(struct pot a,struct pot b){
...
unsigned long long int cal_dis2(struct pot a,struct pot b){
...
int main(){
    ...
    int k=1,step1=63,step2=0,step3=0;
    unsigned long long int dis,h=1ULL<<63;
    while(k<n){
        dis=cal_dis1(m[k],m[k-1]);
    ...
    a1=cal_dis1(m[1],m[0]);
    b1=cal_dis2(m[1],m[0]);
    if(a1!=0&&(a1+b1==0))printf("18446744073709551616\n%d",step2+step3);
    else   printf("%llu\n%d",a1+b1,step2+step3+1);
    return 0;
}