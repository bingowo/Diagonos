...
void out(int father,int mother,int child,int *ans,int f1,int m1,int c1,int *bns){
    ...
    if(father==4){
        if(sum==0||su==0)printf("impossible %s%s %s%s",key[mother],ke[m1],key[child],ke[c1]);
            ...
            if(!(su==1&&sum==1))printf("{");
            if(su==1){
                for(int i=0;i<4;i++){
                    if(ans[i]!=0&&sum>1){printf("%s%s,",key[i],ke[flag]);sum--;}
                    else if(ans[i]!=0&&sum1!=1)printf("%s%s} ",key[i],ke[flag]);
                    else if(ans[i]!=0)printf("%s%s ",key[i],ke[flag]);
                }
        ...
        if(sum==0||su==0)printf("%s%s impossible %s%s",key[father],ke[f1],key[child],ke[c1]);
            ...
            if(!(su==1&&sum==1))printf("%s%s {",key[father],ke[f1]);
            else printf("%s%s ",key[father],ke[f1]);
            if(su==1){
                for(int i=0;i<4;i++){
                    if(ans[i]!=0&&sum>1){printf("%s%s,",key[i],ke[flag]);sum--;}
                    else if(ans[i]!=0&&sum1!=1)printf("%s%s} ",key[i],ke[flag]);
        ...
        if(sum==0||su==0)printf("%s%s %s%s impossible",key[father],ke[f1],key[mother],ke[m1]);
            ...
            if(!(su==1&&sum==1))printf("%s%s %s%s {",key[father],ke[f1],key[mother],ke[m1]);
            else printf("%s%s %s%s ",key[father],ke[f1],key[mother],ke[m1]);
            if(su==1){
                for(int i=0;i<4;i++){
                    if(ans[i]!=0&&sum>1){printf("%s%s,",key[i],ke[flag]);sum--;}
                    else if(ans[i]!=0&&sum1!=1)printf("%s%s}",key[i],ke[flag]);
...
int main(){
    ...
    int t;
    scanf("%d\n",&t);
    char abc[4][4][5]={{"1001","1110","1111","1001"},{"1110","1110","1110","1010"},{"1111","1110","0011","0011"},{"1001","1010","0011","0001"}};
    char zf[2][2][3]={{"11","11"},{"11","01"}};
    for(int i=0;i<t;i++){
        scanf("%s %s %s",s1,s2,s3);
        int mother,father,child,c[4]={0},f1,m1,c1;
        mother=read(s2,c+1);
        father =read(s1,c);
        child=read(s3,c+2);
        m1=c[1],f1=c[0],c1=c[2];
        int ans[4],bns[2];
        if(child==4){
            for(int i=0;i<4;i++){
