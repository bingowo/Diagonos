...
BigInt add(BigInt a,BigInt b){
    ...
    c.cnt=i+1;
    while(c.cnt-1>=0&&c.v[c.cnt-1]==0)c.cnt--;
    return c;
}
void split(BigInt *a1,BigInt *a2,char *s1){
    ...
    if(c==s1){
        a2->cnt=500;
        for(int i=strlen(s1)-1;i>0;i--)a2->v[a2->cnt-i]=*(s1+i)-'0';
    }
    else if(c==s1+strlen(s1)-1){
        a1->cnt=strlen(s1)-1;
        for(int i=strlen(s1)-1;i>0;i--)a1->v[a1->cnt-i]=*(s1+i-1)-'0';
    }
    else{
        *c=0;
        a1->cnt=strlen(s);
        for(int i=strlen(s)-1;i>=0;i--)a1->v[a1->cnt-i-1]=*(s1+i)-'0';
        s=c+1;
        *c='.';
        a2->cnt=500;
...
int main(){
    ...
    c1=add(a1,b1);
    c2=add(a2,b2);
    c2.cnt=c2.cnt>500?c2.cnt:500;
    if(c2.v[c2.cnt-n-1]>=5){
        c3.cnt=c2.cnt;
        c3.v[c2.cnt-n]=1;
        for(int i=0;i<c2.cnt-n;i++)c2.v[i]=0;
        c2=add(c2,c3);
    }
    if(c2.cnt>max){carry=c2.v[c2.cnt-1],c2.v[c2.cnt-1]=0;c2.cnt=500;}
    car=Int2Big(carry);
    c1=add(car,c1);
    if(c1.cnt!=0)for(int i=c1.cnt-1;i>=0;i--)printf("%d",c1.v[i]);
    else printf("0");
    printf(".");
