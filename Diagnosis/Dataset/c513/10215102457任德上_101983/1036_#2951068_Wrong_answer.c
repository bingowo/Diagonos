...
int cmp(const void *a,const void *b){
    ...
    x1=m1->x>=0LL?m1->x:LLONG_MAX-(unsigned long long)(m1->x)+1ULL;
    y1=m1->y>=0ll?m1->y:LLONG_MAX-(unsigned long long)(m1->y)+1ULL;
    x2=m2->x>=0ll?m2->x:LLONG_MAX-(unsigned long long)(m2->x)+1ULL;
    y2=m2->y>=0ll?m2->y:LLONG_MAX-(unsigned long long)(m2->y)+1ULL;
    if((x1+y1)==(x2+y2)){
        if(x1==x2){
            if(y1>y2)return 1;
            else return -1;
...
int main(){
    ...
    unsigned long long x4,y4;
    x4=m[0].x>=0LL?m[0].x:LLONG_MAX-(unsigned long long)(m[0].x)+1ULL;
    y4=m[0].y>=0LL?m[0].y:LLONG_MAX-(unsigned long long)(m[0].y)+1ULL;
    unsigned long long int dis=x4+y4;
    while(k<n){
        if(dis%2ULL==0ULL)break;
        else{
            while(Pow(2uLL,step1)<dis)step1++;
                step2+=step1;
                step1=0;
