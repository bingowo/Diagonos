...
int main()
{
    ...
    for(int i=0;i<strlen(str);i++)
    {
        ...
        if(i<strlen(str)-1)
        {
        if(trans[i]==1&&(((trans[i+1]<=10&&trans[i+1]!=1)||trans[i+1]==-1)))
        {
            ans-=1;
            continue;
        }
        if(trans[i]==10&&((trans[i+1]<=100&&trans[i+1]>10||trans[i+1]==-1)))
            {
            ans-=10;
            continue;
        }
        if(trans[i]==100&&((trans[i+1]<=1000&&trans[i+1]>100||trans[i+1]==-1)))
        {
            ans-=100;
            continue;
        }
        if(trans[i]==5&&trans[i+1]==-1) {ans-=trans[i];continue}
        }
        ans+=trans[i];
        }
