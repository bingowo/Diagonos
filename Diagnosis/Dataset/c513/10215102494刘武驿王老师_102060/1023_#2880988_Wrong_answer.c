...
int cmp(void*a,void*b)
{
    ...
    if(((struct str*)a)->test==1&&((struct str*)b)->test==1) return ((struct str*)a)->sum-((struct str*)b)->sum;
    return strcmp(((struct str*)a)->alp,((struct str*)b)->alp);
}
int main()
{
    char c;
    struct str res[1000];
    for(int i=0;i<1000;i++) {res[i].test=0;res[i].sum=0;}
    int i=0,j=0,m=0,n=0;
    while((c=getchar())!=EOF)
    {
        if(c!=' ')
        ...
        else
        {
            ...
            if(res[n].test==1)
            {
                for(int x=0;x<m;x++) res[n].sum+=(res[n].num[x]-'0')*pow(10,m-x-1);
            }
            i=0;
            j=0;
            m=0;
