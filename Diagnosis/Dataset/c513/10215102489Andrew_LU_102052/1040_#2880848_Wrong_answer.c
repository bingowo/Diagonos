	...
	if(c[0]=='?'){
		for(i=0;i<=1;i++){
			if(f[i][0]=='O' && f[1-i][0]=='O')
			 strcpy(c,"{O}");
			else if(f[i][0]=='A' && f[i-1][0]=='O' && f[i][1]!='B'){
				strcpy(c,"{A,O}");
			}
			else if(f[i][0]=='A' && f[i-1][0]=='A' && f[i][1]!='B' && f[1-i][1]!='B') {
				strcpy(c,"{A,O}");
			}
			else if(f[i][0]=='A' && f[i-1][0]=='B' && f[i][1]!='B')
			 strcpy(c,"{A,AB,B,O}");
			else if(f[i][0]=='A' && f[i-1][0]=='A' && f[i][1]!='B' && f[i-1][1]=='B'){
				strcpy(c,"{A,AB,B}");
			}
			...
			if(f[i][0]=='B' && f[1-i][0]=='A' && f[i-1][1]=='B'){
				strcpy(c,"{A,AB,B}");
			}
			else if(f[i][0]=='A' && f[i-1][0]=='O' && f[i][1]=='B'){
				strcpy(c,"{A,B}");
