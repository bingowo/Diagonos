...
int main()
{
    do
    {
        ...
        int l=n>m?n:m;
        for(i=0;i<l;i++){ans[i]+=a[i]-b[i];if(ans[i]<0){ans[i]+=10;ans[i+1]--;}}
        if(ans[i]<0){printf("-");memset(ans,0,1000*sizeof(ans[0]));for(i=0;i<l;i++){ans[i]+=b[i]-a[i];if(ans[i]<0){ans[i]+=10;ans[i+1]--;}}}
        p=ans+l;
        while(*p==0&&p>ans)p--;
