...
typedef struct{char s[16];long long n;}WORD;
...
int main()
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        tab[a[i].s[k++]]=0;
        for(int m=2;a[i].s[k];k++)if(tab[a[i].s[k]]==-1)tab[a[i].s[k]]=m++;
        char val[100];int m=0;
        for(k=0;a[i].s[k];k++)val[m++]=tab[a[i].s[k]]+'0';
        val[m]=0;
        a[i].n=atoll(val);
        //printf("%s!!!\n",val);
    }
