    ...
    else
    {
        ...
        if(lv<=s/2){for(i=lp;i<pos;i++)a[i]=a[pos];s-=lv;
                    for(i=pos;a[i]>=a[i+1];i++);pour(i);}
                          ...
                          for(i=pos;a[i]>=a[i-1];i--);pour(i);}
        else {for(i=pos;a[i]>=a[i+1];i++);int s0=s;s/=2;pour(i);s=s0/2;for(i=pos;a[i]>=a[i-1];i--);pour(i);s=0;
                if(s0%2){for(i=pos;i>=0;i--)if(a[i]==2){b[i][0]=1;b[i][1]=2;break;}for(i=pos;i<=n;i++)if(a[i]==4){b[i][0]=1;b[i][1]=2;break;}}}
