...
struct ab Input()
{
    ...
    for(int i=0;i<(int)strlen(s);i++)
    {
    	...
    	else if(s[i]=='i') AB.b= val==0?f:f*val;
    	if(val!=0 && (s[i+1]=='-'||s[i+1]=='+')) {AB.a=f*val;val=0;f=1;} 
	}
	if(val!=0 && AB.b==0) AB.a=f*val;
	return AB;
}
void Output(struct num N1,struct num N2)
{
	int zero1=0,zero2=0;
    for(zero1=0;N1.L[zero1]==0 && zero1<4000;)zero1++ ;
	for(zero2=0;N2.L[zero2]==0 && zero2<4000;)zero2++ ;
