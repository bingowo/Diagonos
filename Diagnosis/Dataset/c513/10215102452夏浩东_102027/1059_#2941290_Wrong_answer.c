...
int main()
{
    ...
    for(int i=0;i<=k-1;i++)
    {
        ...
        if(rain < 0)
        {
            ...
            num2=(NL[i+1].high-NL[i].high)*NL[i].num;
            printf("%d+%d/%d",res,num1/GCD(num1,num2),num2/GCD(num1,num2));
            return 0;
        }
        res+=NL[i+1].high-NL[i].high;
        NL[i+1].num+=NL[i].num;
        if (rain==0) {printf("%d",res);return 0;}
    }
    if(rain>0)
    {
    	num1=rain%width;
    	...
    	else printf("%d+%d/%d",res,num1/GCD(num1,num2),num2/GCD(num1,num2));
	}
    free(L);
    free(NL);
    return 0;
