...
int main()
{
    ...
    while(scanf("%s %s",s1,s2)!=EOF)
    {
        ...
        if(flag==1){
            int zero=0; 
            while(L1[zero]==0 && zero<600) zero++;
            if(zero==600) printf("0");
            else{for(int i=zero;i<600;i++) printf("%d %d",L1[i],zero);}
            printf("\n");
        }
        else{
            int zero=0;
            while(L2[zero]==0 && zero<600) zero++;
            if(zero==600) printf("0");
            else{ printf("-");
            for(int i=zero;i<600;i++) printf("%d %d",L2[i],zero);}
            printf("\n");
        }
    }
    return 0;
