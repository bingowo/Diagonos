...
int main()
{
    ...
    int xmin=L[0].dx,xmax=L[0].dx,ymin=L[0].dy,ymax=L[0].dy;
    for(int i=0;i<len;i++)
    {
        xmin=L[i].dx<xmin ? L[i].dx:xmin;
        xmax=L[i].dx>xmin ? L[i].dx:xmax;
        ymin=L[i].dy<xmin ? L[i].dy:ymin;
        ymax=L[i].dy>xmin ? L[i].dy:ymax;
    }
    for(int mm=0;mm<cnt;mm++)
    {
        int x=0,y=0,xflag=1,yflag=1,flag=0;
        scanf("%d %d",&x,&y);
        while(1)
        {
            for(int i=0;i<len;i++){
                if(L[i].dx==x && L[i].dy==y) {flag=1;break;}
                else if(L[i].dx!=x) xflag=0;
                else if(L[i].dy!=y) yflag=0;
            }
