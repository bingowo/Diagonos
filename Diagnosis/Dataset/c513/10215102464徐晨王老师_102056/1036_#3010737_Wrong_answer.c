...
long long int calculate(long long int x1,long long int y1,long long int x2,long long int y2){
    ...
    if(z<myababs(x1,x2)||z<myababs(y1,y2))return 64;
    if(z%2==0){
...
int main(){
    ...
    if((myababs(p[0].x,p[1].x)+myababs(p[0].y,p[1].y))<myababs(p[0].x,p[1].x)||(myababs(p[0].x,p[1].x)+myababs(p[0].y,p[1].y))<myababs(p[0].y,p[1].y)){
        printf("18446744073709551616\n");
    }
    else{
        printf("%llu\n",myababs(p[0].x,p[1].x)+myababs(p[0].y,p[1].y));
    }
    printf("%llu",sum);
}
