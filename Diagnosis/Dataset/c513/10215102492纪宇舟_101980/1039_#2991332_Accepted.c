	...
	while(a[--cnt][0]!='('&&cnt!=0)
	{
		if(a[cnt][0]>='a'&&a[cnt][0]<='z')
		{
			...
			if(a[cnt-1][0]=='/')
			{
				...
				if(a[cnt][0]=='-')
				{
					...
					num[findx]=1;var[findx][1]=cur_num;
				}
				else if(a[cnt][0]=='+')
				{
		...
		else if(a[cnt][0]>='0'&&a[cnt][0]<='9')
		{
			if((a[cnt-1][0]>='0'&&a[cnt-1][0]<='9')||a[cnt-1][0]=='=')
			{
				...
				if(zancuncnt>=2){vecnum=0;for(int i=1;i<=num[findx];i++)vec[++vecnum]=var[findx][i];return_type=findx;}
			} 
			else if(a[cnt+1][0]=='*'){cur_num=atoll(a[cnt])*(cur_num+ans);ans=0;}//{ans*=atoll(a[cnt]);cur_num=0;}
			else cur_num=atoll(a[cnt]);//printf("%lldfuck",atoll(a[cnt]));
		}
		else if(a[cnt][0]=='-')
		{
			if(a[cnt-1][0]>='a'&&a[cnt-1][0]<='z')
			{
				...
				{
					vecnum=0;
					for(int i=1;i<=num[findx];i++)vec[++vecnum]=var[findx][i]-(cur_num+ans);
					return_type=findx;
				}
			}
			else 
		...
		{
			if(a[cnt+1][0]=='*'){cur_num=cal_first()*(cur_num+ans);ans=0;}//{ans*=cal_first();cur_num=0;}
		...
		else if(a[cnt][0]=='=')
		{
			...
			if(return_type!=0)
			{
				num[findx]=num[return_type];
				for(int i=1;i<=num[findx];i++)var[findx][i]=vec[i];
			}
			else {num[findx]=1;var[findx][1]=cur_num;}findx=find();//printf("fuck %lld %lld\n",cur_num,findx);
		}
	}ans+=cur_num;
	if(return_type!=0)
	{
