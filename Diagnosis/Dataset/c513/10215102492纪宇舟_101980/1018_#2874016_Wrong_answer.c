...
void cal()
{
	...
	//for(int i=cnt2;i>=1;i--)printf("%d ",compu[1]);printf("\n");
	...
	for(int i=L+cnt2+5;i>=1;i--)if(compu[i]!=0||i==1){cnt2=i;break;}
	//for(int i=cnt2;i>=1;i--)printf("%d ",compu[i]);printf("\n");
	if(cnt2>cnt1)cnt1=cnt2;
	for(int i=1;i<=cnt2;i++)ans[i]+=compu[i];
	...
	for(int i=cnt2+5;i>=1;i--)if(prisum[i]!=0){cnt2=i;break;}//´¦ÀíËØÊý³Ë»ý 
