...
int arr[1005];
void full_final(Valley s,int l,int r,int dir)//计算不满时分数 
{
	int t=0,cnt;
...
Valley full_sub(Valley s,int l,int r,int dir)//一个小坑一个小坑填水 
{
	int t=l;
	while(t!=r&&a[t+dir].v<=a[t].v)t+=dir;//找坑 
...
int water(int s,int p)
{
	...
	if(suml+sumr==0)//当前是坑中心 直接倒水 
	return water_lowest(s,a[p].v,l-1,r+1);
	else if(suml+sumr<=s)//左右可以放水到与当前位置齐平 
	{
		for(int i=l;i<=r;i++)a[i].v=a[p].v;
		return s-suml-sumr;//再来一次 
	} 
	else if(suml*2<=s)//左边放满 流到右边不满 
	{
