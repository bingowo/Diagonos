...
int main()
{
    ...
    memset(a.num,0,sizeof(a.num));memset(b.num,0,sizeof(b.num));
    memset(qr.num,0,sizeof(qr.num));memset(qi.num,0,sizeof(qi.num));
    for(int i=2;i<L;i++)
    {
        int x;
    ...
    if(b.cnt!=1||b.num[1]!=0)if(b.cnt==1&&b.num[1]==1)printf("i");else if(b.sig==-1&&b.cnt==1&&b.num[1]==1)printf("-i");else {print(b);printf("i");};
    return 0;
}