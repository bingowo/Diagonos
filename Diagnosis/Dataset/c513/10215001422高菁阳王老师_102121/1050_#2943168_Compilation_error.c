...
int main(int argc, char *argv[]) {
	...
	for(k=0;k<n;k++){
		scanf("%s\n%s\n%s\n",s1,s2,s);
		printf("case #%d:\n",k);
		x1=FindSubstrFromLeft(s,s1);printf("x1=%d\n",x);
		y1=FindSubstrFromRight(s,s2);printf("y1=%d\n",y);
		if (x!=-1&&y!=-1) 
  {  max=Max(max,y1-x1-strlen(s1));
      x2=FindSubstrFromLeft(s,s2);printf("x2=%d\n",x);
      y2=FindSubstrFromRight(s,s1);printf("y2=%d\n",y);
