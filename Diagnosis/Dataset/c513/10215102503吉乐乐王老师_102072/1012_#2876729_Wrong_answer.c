...
prural get(){
    ...
    if(s[0]=='-'){
        ...
        num1[j]='\0';
        if(s[i]=='i'){a.imaginary=atoi(num1);return a;}
        if(s[i]=='\0'){a.real=atoi(num1);return a;}
        char num2[100];
        while(s[i]){
            if(s[i+1]=='i'){a.real=atoi(num1);a.imaginary=s[i]=='-'?-1:1;return a;}
            while(s[i]){num2[k]=s[i];i++;k++;if(s[i]=='i')break;}
        }
        a.real=atoi(num1);a.imaginary=atoi(num2);
    }
    if(isdigit(s[0])){
...
int main(){
    ...
    int rmd;
    if(ini.real==0&&ini.imaginary==0)ret[0]==0;
