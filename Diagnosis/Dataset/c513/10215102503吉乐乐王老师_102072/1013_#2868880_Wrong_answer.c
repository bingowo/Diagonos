...
int main(){
    ...
    if(fenshu<0){fenshu+=get(3,cntj),zhengshu-=1;zhengshu=-zhengshu;};
    if(fenshu!=0){
        int GCD=gcd(fenshu,get(3,cntj));
        if(zhengshu==0)printf("%d %d",fenshu/GCD,get(3,cntj)/GCD);
        else printf("%d %d %d",zhengshu,fenshu/GCD,get(3,cntj)/GCD);}
    else printf("%d",zhengshu);
}