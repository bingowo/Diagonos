...
void child(char fa[],char mo[],int ret[]){
    ...
    if(ch1=='+'||ch2=='+'){ret[4]=1;ret[5]=1;return ;}
    if(ch1=='-'&&ch2=='-'){ret[4]=0;ret[5]=1;return ;}
}
...
void par(char ch[],char pa[],int ret[]){
    ...
    if(oth==0){ret[0]=-1;return;}
    if(strcmp(ch,"AB")==0&&strcmp(pa,"AB")==0){ret[0]=1;ret[2]=1;}
    if(oth=='A'){
        ret[0]=1;ret[1]=1;
    }
    if(oth=='B'){
...
int main(){
    ...
    while(t--){
        ...
        if(strcmp("?",ch)==0){
            ...
            if(idx==0)printf("%s %s impossible",fa,mo);
            else if(idx==1)printf("%s %s %s",fa,mo,ans[0]);
            else {
                printf("%s %s {",fa,mo);
                for(int i=0;i<idx;i++){
                    printf("%s",ans[i]);
            ...
            if(ret[0]==-1){
                ...
                if(idx==0)printf("impossible %s %s",mo,ch);
                else if(idx==1)printf("%s %s %s",ans[0],mo,ch);
                else {
                    putchar('{');
                    for(int i=0;i<idx;i++){
                        printf("%s",ans[i]);
            ...
            if(ret[0]==-1){
                ...
                if(idx==0)printf("%s impossible %s",fa,ch);
                else if(idx==1)printf("%s %s %s",fa,ans[0],ch);
                else {
                    printf("%s {",fa);
                    for(int i=0;i<idx;i++){
                        printf("%s",ans[i]);
