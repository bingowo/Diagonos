...
int main(){
    ...
    for(int i=0;i<n;i++){
        ...
        a[i].dis=llabs(a[i].dx)+llabs(a[i].dy);
    }
    qsort(a,n,sizeof(a[0]),cmp);
    int cnt=0;
    for(int i=1;i<n;i++){
        unsigned long long int x=llabs(a[i].dx-a[i-1].dx);
        unsigned long long int y=llabs(a[i].dy-a[i-1].dy);
        int flag=0;
        while(x||y){
            ...
            if(isodd(y)){
                x/=2;y/=2;cnt+=1;
                if(x==0&&y==0)break;
                if((!isodd(x)&&!isodd(y))||(isodd(x)&&isodd(y)))x+=1;
                continue;
            }
        }
        if(flag)break;
    }
    unsigned long long int d1=llabs(a[1].dx-a[0].dx);
    unsigned long long int d2=llabs(a[1].dy-a[0].dy);
    int r1[N]={0},r2[N]={0},ret[N]={0};
    myultoa(r1,d1);myultoa(r2,d2);
    add(ret,r1,r2);
    int i=0;
    while(i<N&&ret[i]==0)i++;
    if(i==N)printf("0");
    else while(i<N)printf("%d",ret[i++]);
    printf("\n%d",cnt);
}