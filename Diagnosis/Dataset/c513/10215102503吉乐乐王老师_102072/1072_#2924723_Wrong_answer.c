...
int main(){
    ...
    int cnt=0;
    while(!((nx==x)&&(ny==y))){
        dx=abs(x-nx)/base;dy=abs(y-ny)/base;
        if(imp(dx,dy)){flag=-1;break;}
            ...
            if(dx&1){
                ...
                long int dx1=abs(x-trywest)/base;
                long int tryeast=nx+nstep;
                long int dx2=abs(x-tryeast)/base;
                dy=abs(y-ny)/base;
                if(!imp(dx1,dy)&&(dx2!=0)){
            ...
            if(dy&1){
                ...
                long int dy1=abs(y-trysouth)/base;
                long int trynorth=ny+nstep;
                long int dy2=abs(y-trynorth)/base;
                dx=(x-nx)/base;
                if(!imp(dx,dy1)&&(dy2!=0)){
