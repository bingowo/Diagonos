...
int main(){
    ...
    if(baseima==0){
        ...
        if((sign==1&&n/2%2)||(sign==-1&&n/2%2==0))putchar('-');
        if(baseima==1&&n%2){putchar('i');}
        else{
            for(int i=0;i<N;i++){
        ...
        int flag1=0;
        if(flag){
            for(int i=N-1;i>0;i--)if(retb[i]!=0){flag1=1;break;}
            if(flag1)
                if(retb[0]==0)putchar('+');
        }
        if(flag1&&retb[0]==-1)putchar('-');
        int idx=1;
