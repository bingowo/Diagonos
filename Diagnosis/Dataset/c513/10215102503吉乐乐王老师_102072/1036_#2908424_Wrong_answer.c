...
int main(){
    ...
    for(int i=0;i<n;i++){
        ...
        a[i].dis=myabs(a[i].dx);
        a[i].dis+=myabs(a[i].dy);
    }
    qsort(a,n,sizeof(a[0]),cmp);
    int cnt=0;
    for(int i=1;i<n;i++){
        unsigned long long int x=myabs(a[i].dx-a[i-1].dx);
        unsigned long long int y=myabs(a[i].dy-a[i-1].dy);
        int flag=0;
        while(x||y){
    ...
    unsigned long long int retdis=(unsigned long long int)(a[0].dx-a[1].dx)+(unsigned long long int)(a[0].dy-a[1].dy);
    printf("%llu\n%d",retdis,cnt);
}