...
int main() {
	char s[1000];
	int number=0,i,j,location,up=1,down=1;
	scanf("%s",s);
	if(strchr(s,'.')==NULL){
		...
		if(number>0){
			...
			printf("%d %d %d",number,up/gcd(up,down),down/gcd(up,down)); 
		}
		else if(number<0){
			if(up>0){
				number++;up=down-up;
			...
			if(up>0)printf("%d %d",up/gcd(up,down),down/gcd(up,down));
			else printf("%d %d",up/gcd(-up,down),down/gcd(-up,down));
		}
