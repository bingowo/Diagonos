...
long long int zhishu(long long int x){
	long long int i=1;
	while(x>0){
		i*=3;x--;
	}
...
int main() {
	...
	long long int i,j,result,x,len,len1,a,k,result1=0,result2=0;
	scanf("%s",s);
	if(strchr(s,'.')==NULL){
		...
		if(result==0)printf("%lld %lld",result1/a,result2/a);
		else printf("%lld %lld %lld",result,result1/a,result2/a);
	}
	return 0;
}