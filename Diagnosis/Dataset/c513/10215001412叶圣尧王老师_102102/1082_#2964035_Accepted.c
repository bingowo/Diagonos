...
int main()
{
    ...
    {   int d1=0,d2;
        ...
        for(i=k-1;i>=0;i--)
        {   d2=(out_f[i]-'0'+d1)/10;
            out_f[i]=(out_f[i]-'0'+d1)%10+'0';
            d1=d2;
        }out_f[k]='\0';
        d+=d1;
    }
    for(i=len_da-1,j=len_db-1;j>=0&&i>=0;i--,j--)
    {
        out_d[i]=(da[i]-'0'+db[j]-'0'+d)%10+'0';
        d=(da[i]-'0'+db[j]-'0'+d)/10;
    }out_d[len_da]='\0';
    for(;i>=0;i--)
    {
        out_d[i]=(da[i]-'0'+d)%10+'0';
        d=(da[i]-'0'+d)/10;
    }
    if(d==1)printf("1");
    i=0;
    while(d==0&&out_d[i]=='0'&&out_d[i+1]!='\0')i++;
    printf("%s.%s",out_d+i,out_f);
