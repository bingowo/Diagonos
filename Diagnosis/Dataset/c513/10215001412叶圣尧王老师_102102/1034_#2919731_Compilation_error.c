...
int cmp(const void*a,const void*b)
{
    ...
    if(c->f==d->f)
    {
        if(abs(d->chr-c->chr)==32)return d->chr-c->chr;
        else if(d->chr>='a'&&d->chr<='z'&&c->chr>='a'&&c->chr<='z')return c->str-d->str;
        else if(d->chr>='A'&&d->chr<='Z'&&c->chr>='a'&&c->chr<='z')return c->str-d->str-32;
        else return c->str-d->str+32;
    }
