...
int main()
{
	...
	for(int i=0;i<t;i++)
	{
		...
		if(x!=0||y!=0)
		{
		if(x!=0&&y!=0) bushu=(abs(qus[0]/x)>abs(qus[1]/y))?abs(qus[1]/y):abs(qus[0]/x);
		else if(x!=0) bushu=abs(qus[0]/x);
		else if(y!=0) bushu=abs(qus[1]/y);
		...
		else 
		{
			for(int k3=0;k3<l&&a!=1;k3++)
			{
				if(qus[0]==an[k3][0]&&qus[1]==an[k3][1])
				{
					a=1;
					break;
				}
