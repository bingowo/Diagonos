...
Vector ADD(Vector m1,Vector m2)
{
	...
	{
		tmp=m2;
		for(int i=0;i<m2.cnt;i++) tmp.data[i]+=m1.data[0];
	}
	else if(m2.cnt==1)
	{
		tmp=m1;
		for(int i=0;i<m1.cnt;i++) tmp.data[i]+=m2.data[0];
	}
	return tmp;
}
Vector SUB(Vector m1,Vector m2)
{
	...
	{
		tmp=m2;
		for(int i=0;i<m2.cnt;i++) tmp.data[i]=m1.data[0]-tmp.data[i];
	}
	else if(m2.cnt==1)
	{
		tmp=m1;
...
Vector calc(int l,int r)
{
	...
	{
		opera1=calc(j+2,r);
		if(!strcmp(item[j],"+")) return compADD(opera1);
		if(!strcmp(item[j],"-")) return compSUB(opera1);
		if(!strcmp(item[j],"*")) return compMUL(opera1);
	}
}
int main()
{
