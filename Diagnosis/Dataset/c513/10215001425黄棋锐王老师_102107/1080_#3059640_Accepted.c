...
{//乘法从最低位开始
	int carry=d,i;//carry-进位 ，d为0时任然有效，此题d和carry都肯定为0或1，不必乘其他系数
	for (i=0;i<n->cnt;i++) {//后面加一位数，直接d+个位*2，已有的位的数都要乘2
...
{   int k,T;
 ...
 {  char line[102]={'\0'};
 	...
 	for (i=0;i<cnt;i++) MUL2ADD(&n,er[i]);//二转十，从二的最高位开始算，高位0用循环干掉
