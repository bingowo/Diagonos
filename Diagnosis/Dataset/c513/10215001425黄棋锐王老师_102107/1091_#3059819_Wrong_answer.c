...
{ char s[2000]={'\0'};
	...
	char*p;//******************************处理输入问题 
	 ...
	 int signr=qr/(abs(qr)),signi=qi/(abs(qi));
	  qr=abs(qr),qi=abs(qi);
	  int SR=signr,SI=signi,sign1,sign2,sign3=signr,sign4=signi,sign5,sign6;
		BIGINT QR=int2BIG(qr),QI=int2BIG(qi),R=QR,I=QI,F1,F2,aa,bb,cc=QR,dd=QI;//************************* 
		int i;
		for(i=1;i<n;i++) 
		{	char s1[2000]={'\0'};char s2[2000]={'\0'};char s3[2000]={'\0'};char s4[2000]={'\0'};
