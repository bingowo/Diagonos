...
{  int i,l=strlen(s);
	...
	{ if(s[x]==s[x-1]&&s[x]==s[x+1]) return int2BIG(1);//aaa
		else if(s[x]==s[x-1]&&s[x]!=s[x+1])return int2BIG(2);//aab
		else if(s[x]!=s[x-1]&&s[x]==s[x+1]) return int2BIG(2);//abb
		else if(s[x]!=s[x-1]&&s[x]!=s[x+1]&&s[x-1]==s[x+1]) return int2BIG(2);//aba
		else if (s[x]!=s[x-1]&&s[x]!=s[x+1]&&s[x-1]!=s[x+1]) return int2BIG(3);//abc
	}
}
int main() 
{ char s[2000]={'\0'};
