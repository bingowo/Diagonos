 ...
 while (p < end)      /*当指针没有到达文件末尾 */
 {
  ...
  switch (c)             //根据c的值做相应处理
  {
   case '\'':              /*处理单引号*/
   {
    ...
    {
     sq_start = p++;            //start指向单引号的开始位置，p指向下一个</p><p style="margin-top:0px; margin-bottom:0px; padding-top:0px; padding-bottom:0px; color:rgb(51,51,51); font-family:tahoma,arial,宋体,sans-serif; font-size:15px; line-height:18px">    }
    else                              /*如果遇到过单引号，sq_start指向单引号开始位置*/
    {
     len = (p++) -sq_start;    
    if (len == 2 && *(sq_start+1) == '\\'){     /*若遇到 “  '\''   ”这种情况则两个单引号并未匹配，遇到的“'”是“\' ”中的，而不是与sq_start所指向单引号匹配*/
      continue;
     }
   ...
   case '\"':                   /*处理双引号*/
   {
    ...
    else if (*((p++) -1) =='\\')     /*若遇到 “" ab\''cd"   ”这种情况则两个双引号并未匹配，遇到的“"”是“\"”中的，而不是与dq_start所指向双引号匹配*/
     continue;
    }
   ...
   case '\n': /*换行符，主要处理遇到双斜杠时，需要清除双斜杠到\n的前面的字符*/
   {
    ...
    c = *(p - 1);
    /*如果遇到过双斜杠，清空双斜杠本身和到\n前面的那个字符，p指向下一个字符，/r是回车符（光标退回到最前面），这里要判断</p><p style="margin-top:0px; margin-bottom:0px; padding-top:0px; padding-bottom:0px; color:rgb(51,51,51); font-family:tahoma,arial,宋体,sans-serif; font-size:15px; line-height:18px">   c == '\r'是因为在UNIX系统下文件结尾的换行只有\n，而windows系统下文件结尾的换行为\r\n   */</p><p style="margin-top:0px; margin-bottom:0px; padding-top:0px; padding-bottom:0px; color:rgb(51,51,51); font-family:tahoma,arial,宋体,sans-serif; font-size:15px; line-height:18px">    memset(lc_start, ' ', (c == '\r'? ((p++) -1) : p++) - lc_start); 
    lc_start = NULL;
    break;
   }
