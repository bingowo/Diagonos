...
int add1(BIG*a,int k)//在第k位上+1，并且返回数字变化的最高位 
{	int i=k;
...
{   int k,T;
	...
	{   char A[MAXN]={'\0'};
		...
		ctB(A,&a);//转换 
		i=add1(&a,0);//a+1，
		for(i=a.cnt-1;i>=0;i--)//查看是否是重复数，从可能的次高位开始
		{
			if(a.v[i]==a.v[i+1]) break; 
		}
		
		if(i!=-1)//重复数 
		{	i=add1(&a,i);//在重复数字的最高处的第二位+1，返回发生数字变化的最高位，如1100->1200 
			while(i<a.cnt)//对数字变化最高位前面的可能重复进行循环处理 ,至多到a的最高位 
			{    if(a.v[i]!=a.v[i+1]) break;//数字变化最高位i与i+1位不同就没有重复 
				i=add1(&a,i);
			}
			int j;
			for(j=i-1;j>=0;j--)//交替添加0和1，保证最小 如1100->1200->1201
			{
				a.v[j]=0+(i-j+1)%2;
			}
		}
