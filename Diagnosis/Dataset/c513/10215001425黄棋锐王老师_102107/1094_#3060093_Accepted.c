...
{	BIGINT aa=*(BIGINT*)a;
	...
	if(aa.sign==bb.sign)
	{ if(aa.sign==1) return stcmp(sa,sb);
		else return stcmp(sb,sa);
	}
...
{ 
	...
	{	if(stcmp(sa,sb)>0) ans=A_B(&small,&big);
		else if(stcmp(sa,sb)<0) ans=A_B(&big,&small);
		else if(stcmp(sa,sb)==0) ans.cnt=1,ans.v[0]=0;
	}
	else
	{
		ans=AaddB(&small,&big);
