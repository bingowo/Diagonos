...
FLOAT CTF(char *a);
void PLUS(FLOAT a,FLOAT b,int n);
FLOAT trans(FLOAT a);
int main()
{
   int n,z;
...
void PLUS(FLOAT a,FLOAT b,int n)
{
    ...
    for(k=a.cntd-1;k>=0;k--)
    {
        ...
        if(t>9){t=t-10;record=1;res.d[k]=t;}
    ...
    {
        k=b.cnti;
        for(;k<a.cnti;k++){b.i[b.cnti++]=0;}
    }
    if(a.cnti<b.cnti)
    {
        k=a.cnti;
        for(;k<b.cnti;k++){a.i[a.cnti++]=0;}
    }
    for(k=0;k<b.cnti;k++)
    {
        t=a.i[k]+b.i[k]+record;
        if(t>9){t=t-10;record=1;res.i[res.cnti++]=t;}
        else {record=0;res.i[res.cnti++]=t;}
        //printf("(%d)",res.i[k]);
    }
    if(record==1){res.i[res.cnti++]=record;record=0;}

    /*k=res.cnti-1;
    while(res.i[k]==0&&k>0)k--;
    if(k==0&&res.i[k]==0)printf("0");
    else
    {
