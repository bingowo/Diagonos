...
{
    char s[200]={0};
    TIME* arr=(TIME*)malloc(N*sizeof(TIME));int cnt=0;
    while (1)
    {
        gets(s);
        if (strcmp(s,endd)==0)
        ...
        if (strcmp(subs,ID)==0)
        {
            ...
            x=x^s[i];    for (i=i+1;s[i]!=',';i++) x=x^s[i];
            x=x^s[i];
            ...
            if (state=='A')
            {
                ...
                if (x==d)
                {
                    ...
                    if (hour>=24)
                        hour-=24;
                    arr[cnt].s[0]=hour/10+'0';
                    arr[cnt].s[1]=hour%10+'0';
                    cnt++;
                }
            }
        }
