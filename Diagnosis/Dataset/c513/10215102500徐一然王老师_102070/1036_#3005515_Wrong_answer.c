...
int main()
{
    ...
    for(int i=0;i<n-1;i++)
    {
        ...
        {
            if(A[i].x<=0&&A[i+1].x<=0){if(A[i].x<A[i+1].x){t1=-A[i].x+A[i+1].x;}else{t=A[i].x-A[i+1].x;}}
            else if(A[i].x>=0&&A[1+i].x<=0)t=A[i].x-A[i+1].x;
            else if(A[i].x<=0&&A[1+i].x>=0)t=-A[i].x+A[i+1].x;
            else {if(A[i].x<A[i+1].x){t1=-A[i].x+A[i+1].x;}else{t=A[i].x-A[i+1].x;}}

            if(A[i].y<=0&&A[i+1].y<=0){if(A[i].y<A[i+1].y){t=-A[i].y+A[i+1].y;}else{t1=A[i].y-A[i+1].y;}}
            else if(A[i].y>=0&&A[1+i].y<=0)t1=A[i].y-A[i+1].y;
            else if(A[i].y<=0&&A[1+i].y>=0)t1=-A[i].y+A[i+1].y;
            else {if(A[i].y<A[i+1].y){t=-A[i].y+A[i+1].y;}else{t1=A[i].y-A[i+1].y;}}
            t=t1+t;
            if(t%2==0)break;
            t=t+1;
            if((t-2)%4!=0);
