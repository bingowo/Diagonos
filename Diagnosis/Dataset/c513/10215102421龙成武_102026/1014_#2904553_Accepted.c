...
void threeminu(char*res,int flag)
{
    int a;
    for(int i=flag;i<strlen(res);i++)
    {
        a=res[i]-'0'-1;
        if(a==-1)
        {
...
int main()
{
    ...
    temp=strlen(res);
    p=res;
    if(res[0]=='0'&&res[1]!='0'&&res[1]!='.'&&res[1]!=0)
    {
        for(;*p=='0';p++);
    }
    strcpy(res2,p);
