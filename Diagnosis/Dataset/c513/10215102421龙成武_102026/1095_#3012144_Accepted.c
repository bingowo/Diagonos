...
void BigIntMul(pBIGINT num1, pBIGINT num2, pBIGINT res)
{
    ...
    BigIntTrim(res);
    res->sign = num1->sign * num2->sign;
    if (num1->digit == 1 && num1->num[0] == 0 || num2->digit == 1 && num2->num[0] == 0) res->sign = 1;
}
int main()
{
    char s[3000];
    ...
    if (flaga == 0 && flagb == 0) printf("0");
    return 0;
}
//只能说江晖那个1014题的i-1进制的思路挺好用的，乘以-4