...
void BigFloatAdd(pBIGINT num1, pBIGINT num2, pBIGINT res)
{
    int i;
    res->sign = num1->sign;   //保存符号
    ...
    res->digit = num1->digit + 2;
    BigIntTrim(res);
    if (res->num[num1->digit] != 0)
        res->digit = num1->digit + 1;
    else
        res->digit = num1->digit;
}
int main()
{
    char A[1000], B[1000];
    ...
    BigFloatAdd(&fnum1, &fnum2, &fres);     //也是因为浮点数不能简单去除前导零

    if (fnum1.digit >= N + 1)            //处理四舍五入
    {
        i = fnum1.digit - N - 1;        //鉴于fres.digit>=fnum1.digit
        if (fres.num[i] >= 5)
        {
            fres.num[i] = 0;
            fres.num[++i] ++;
        ...
        //BigIntTrim(&fres);        浮点数的前导零不能随便清除的
    }
