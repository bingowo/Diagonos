...
int ismiao(int* a, int len)
{
    ...
    if (flag == 1 && sum % 9 == 0) flag = 0;
    return flag;
}
int findnine(int* a, int len)  //返回位权最大的9在a里的下标
{
    for (len--;len > -1;len--)
        if (a[len] == 9) break;
    return len;  //返回-1代表没有9
}
int main()
{
    int len,i;
    ...
    {
        if (findnine(num1.num, num1.digit) != -1)           //这是一个优化，靠硬加1会在某些恶心数据上超时
        {
            for (int j = 0;j < findnine(num1.num, num1.digit);j++)
            {
                num1.num[j] = 0;
            }
            num2.digit = findnine(num1.num, num1.digit) + 1;
