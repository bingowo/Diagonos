...
{ int T,a[11],i,j,k,m,n,l,count;
 ...
 { for(j=0;j<9;j++)
   ...
   { if(a[j]==0) {count++;continue;} 
      else if(a[j]==1&&count==j) printf("x^%d",8-j);
      else if(a[j]==1&&count!=j) printf("+x^%d",8-j);
      else if(a[j]==-1) printf("-x^%d",8-j);
      else if(a[j]>1&&count==j) printf("%dx^%d",a[j],8-j);
      else if(a[j]>1&&count!=j) printf("+%dx^%d",a[j],8-j);
      else if(a[j]<-1) printf("%dx^%d",a[j],8-j);
   }
   if(a[7]==0) {count++;} 
      else if(a[7]==1&&count!=7) printf("+x");
      else if(a[7]==1&&count==7) printf("x");
      else if(a[7]==-1) printf("-x");
      else if(a[7]>1&&count!=7) printf("+%dx",a[7]);
      else if(a[7]>1&&count==7) printf("%dx",a[7]);
      else if(a[7]<-1) printf("%dx",a[7]);
   if(count==8&&a[8]==0) printf("0");
   else if(a[8]>0&&count!=8) printf("+%d",a[8]);
   else if(a[8]>0&&count==8) printf("%d",a[8]);
