...
{ struct count d1,d2;
  ...
  d2=*((struct count*)b);
   ...
   else if(d1.digit>d2.digit) return 1;
   else if(d1.digit==d2.digit)
   { if (d1.win>d2.win) return -1;
     else if (d1.win<d2.win) return 1;
     else if  (d1.win==d2.win)
     {if (d1.lose>d2.lose) return 1;
     else if (d1.lose<d2.lose) return -1;
...
{ if ((n==0)&&(m==0)) break;
  for(i=0;i<m;i++)
{ scanf("%d %d %d",&a,&b,&c);
  if(c==1) {team[a-1].digit+=3;team[a-1].win+=1;team[b-1].lose+=1;}
  if(c==0) {team[a-1].digit+=1; team[b-1].digit+=1;}
  if(c==-1) {team[b-1].digit+=3;team[b-1].win+=1;team[a-1].lose+=1;}
}
qsort(team,n,sizeof(team[0]),cmp);
for (i=0;i<n-1;i++)
{ printf("%d ",team[i].digit);
}
printf("%d\n",team[n-1].digit);
for (x=0;x<1000;x++)
{team[x].number=x+1; team[x].digit=0;team[x].win=0;team[x].lose=0;}
