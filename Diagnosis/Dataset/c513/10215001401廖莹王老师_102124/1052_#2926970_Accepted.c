...
#include <stdlib.h>
#include<string.h>
void shandiao(char s[10],char s2[100])
{
      int i=0;
      char t[2]={0};
      for(i=0;s2[i];i++)
...
int mima(char s[30],int a[30])
{
    ...
    for(i=0;s[i];i++)
    {
        switch(s[i])
        {
            ...
            case 'O':a[k]=1;k++;shandiao(s,"ONE");break;
            default:break;
        }
    }
    for(i=0;s[i];i++)
