...
int main()
{
    ...
    while(scanf("%s",s)!=EOF)
    {
        char tu[42][42]={0};
    int i=0,j=0;
    for(i=0;i<42;i++){for(j=0;j<42;j++)tu[i][j]='.';}
    for(i=0;i<42;i++){if(i==0)tu[i][20]='^';else tu[i][20]='|';}
    for(i=0;i<42;i++){if(i==41)tu[20][i]='>';else tu[20][i]='-';}
    tu[20][20]='+';
    ...
    for(m=0;m>=-20;m--){g=0;g=p[0]+p[1]*m+p[2]*m*m+p[3]*m*m*m;if(abs(g)<=20){tu[20-g][20+m]='*';}}
    for(i=0;i<42;i++){for(j=0;j<42;j++)printf("%c",tu[i][j]);printf("\n");}
