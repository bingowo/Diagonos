...
int main()
{
   char s[10000]={0};
   scanf("%s",s);
   long long i=0,k=0,d,j,m,h=0;int t[10000];
   point qr,qi,a,b;
...
if(a.a[0]==0 &&b.a[0]!=0 ){if(b.f==-1)printf("-"); if(b.cnt==1 && b.a[0]==1)printf("i");else{for(h=b.cnt-1;h>=0;h--)printf("%d",b.a[h]);printf("i");}}
else
{
    if(b.a[0]==0){if(a.f==-1)printf("-");for(h=a.cnt-1;h>=0;h--)printf("%d",a.a[h]);}
    else
    {
