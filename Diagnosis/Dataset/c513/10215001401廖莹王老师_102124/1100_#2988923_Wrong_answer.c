...
int pd(int a,int b,int l)
{
    ...
    {   if(x1==0 && x!=0)return -1;
        else
        {   if(x1!=0 && x==0 )return -1;
            else
            {
            if(x1!=0 && x!=0){m=x%x1;if(m!=0)return -1;}
            if((y>=0 && y1>=0)|| (y<=0 && y1<=0))
            {
            if(y1==0 && y!=0)return -1;else {if(y1!=0 && y==0)return -1;else{if(y1!=0 && y!=0){n=y%y1;if(n!=0)return -1;}else return 1;}}
            }
            else return -1;
            }
        }
