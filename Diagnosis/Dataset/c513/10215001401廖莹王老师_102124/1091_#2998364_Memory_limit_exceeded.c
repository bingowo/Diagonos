...
{
    int i=0,j=0,m=0,n=0;
    point c;c.cnt=0;memset(c.a,0,sizeof(int)*200);
    c.f=a.f;
    if(a.cnt>b.cnt)
    {
        for(i=0;i<b.cnt;i++)
...
int main()
{
    int n,i,f=1,m=0,k=0;char s[1000]={0};
    point sum1={0,1,{0}};point sum2={0,1,{0}};
    scanf("%s",s); scanf("%d",&n);
    for(i=0;i<1100;i++){for(k=0;k<1100;k++){ p[i][k].cnt=0;p[i][k].f=1;memset(p[i][k].a,0,sizeof(int)*200);}}
