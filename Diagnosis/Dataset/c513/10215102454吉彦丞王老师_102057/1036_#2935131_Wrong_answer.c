...
int main()
{
    ...
    for(int i=0;i<number;i++)
    {
    	...
    	{
    		long long int f=zhishu(fuck(dx(p[i],p[i+1])+dy(p[i],p[i+1])))-1-dx(p[i],p[i+1])-dy(p[i],p[i+1]);
        	while(f>0)
        	{
			...
			if(f==0)
			{
				t=t+fuck(dx(p[i],p[i+1])+dy(p[i],p[i+1]));
			}
			else
			{
				break;
