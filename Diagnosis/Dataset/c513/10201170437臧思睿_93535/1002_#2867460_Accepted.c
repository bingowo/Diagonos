...
int main()
{
    ...
    while(t++<T)
    {
        ...
        scanf("%s",s);
        s[strlen(s)]='\0';
        int num=(R(s,strlen(s))==1)?2:R(s,strlen(s));
        for(i=0;i<65;i++)
        {
        ...
        {
            if(s[i]<='9'&&s[i]>='0')
                {sum+=stdi[s[i]-'0']*powx((ll)num,strlen(s)-i-1);}
            if(s[i]<='z'&&s[i]>='a')
                {sum+=stdi[s[i]-'a'+10]*powx((ll)num,strlen(s)-i-1);}
            if(s[i]<='Z'&&s[i]>='A')
                {sum+=stdi[s[i]-'A'+36]*powx((ll)num,strlen(s)-i-1);}
        }
        printf("case #%d:\n%lld\n",t-1,sum);
    }
}
