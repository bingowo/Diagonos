...
int main()
{
    ...
    while(s)
    {
        ...
        else if(!fl&&fr)
        {
            ...
            {
                int d=len*(a[r_pos].fm-a[r_pos].fz);
                if(s>=d)
                {
        ...
        else if(!fr&&fl)
        {
            ...
            if(!a[l_pos].fz)
            {
                if(s>=len)
                {
                    for(i=l_start;i<=l_pos;++i)a[i].h++;
                    dw=len;
                }
                else
                {
                    for(i=l_start;i<=l_pos;++i)a[i]=add(a[i],s,len);
                    dw=s;
                }
            }
            else
            {
                int d=len*(a[l_pos].fm-a[l_pos].fz);
                if(s>=d)
                {
                    for(i=l_start;i<=l_pos;++i)
                    {
                        a[i].h++;
                        a[i].fz=0;
                        a[i].fm=1;
