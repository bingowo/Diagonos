...
{
    int len=strlen(s),i,j,cnt=0;for(i=0;s[i];++i)if(s[i]=='-')cnt++;
    if(cnt==2)
    {
        for(i=1;s[i];++i)if(s[i]=='-')break;
        for(j=len;j>i;--j)s[j]=s[j-1];
        s[i]='+';s[len+1]='\0';
        if(s[len-1]=='-'){s[len]='1';s[len+1]='i';s[len+2]='\0';}
        return;
    }
    ...
    if(!(strchr(s,'+'))&&!(strchr(s,'-')))
    {
        for(j=len+1;j>1;j--)s[j]=s[j-2];
	    s[0]='0';s[1]='+';s[len+2]='\0';//-bi->0+-bi
	    if(s[len]=='+'){s[len+1]='1';s[len+2]='i';s[len+3]='\0';}
        return;
    }
}
...
int main()
{
    char s[20],tmp[10];int p,i,j,k,real,image;scanf("%s %d",s,&p);
    motify(s);
