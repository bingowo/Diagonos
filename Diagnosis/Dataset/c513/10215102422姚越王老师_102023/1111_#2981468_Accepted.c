...
int main()
{
    ...
    while(~scanf("%s",s))
    {
        ...
        for(i=0;i<cnt;++i)
        {
            if(isdigit(s[pos[i]-1])&&s[pos[i]-2]=='^')
            {
                start=i?pos[i-1]:5;
                end=pos[i]-4;
                if(end<start)a[s[pos[i]-1]-'0']=1;
                else
				{
					q=0;for(j=start;j<=end;++j,++q)tmp[q]=s[j];
                	tmp[q]='\0';
                	if(!strcmp(tmp,"-"))a[s[pos[i]-1]-'0']=-1;
                	else if(!strcmp(tmp,"+"))a[s[pos[i]-1]-'0']=1;
                	else a[s[pos[i]-1]-'0']=atoi(tmp);
            	}
            }
            else if(s[pos[i]-1]=='x')
            {
                ...
                if(!isdigit(s[end]))a[1]=(s[end]=='+'||s[end]=='=')?1:-1;
                else
                {
					q=0;for(j=start;j<=end;++j,++q)tmp[q]=s[j];
                	tmp[q]='\0';
