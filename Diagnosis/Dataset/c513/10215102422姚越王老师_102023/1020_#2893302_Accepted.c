...
int cmp2(const void* a,const void* b)//size
{
	...
	else if(ax->s<bx->s)return -1;
	else return strcmp(ax->name,bx->name);
}
int cmp3(const void* a,const void* b)//time
{
	file *ax=(file*)a,*bx=(file*)b;
	if(strcmp(ax->date,bx->date)!=0)return strcmp(ax->date,bx->date);
	else if(strcmp(ax->time,bx->time)!=0)return strcmp(ax->time,bx->time);
	else return strcmp(ax->name,bx->name);
}
int main()
{
	...
	while((scanf("%d",&n))!=EOF)
	{
		...
		else qsort(list,n,sizeof(list[0]),cmp3);//time
		for(int i=0;i<n;i++)
		{
