...
int main()
{
    ...
    while(s)
    {
        ...
        if(fl&&fr)
        {
            int l_end,r_start,lenl,lenr,tmpfz=1,tmpfml,tmpfmr;
            ...
            r_start=i+1;
            lenr=r_pos-r_start+1;
            tmpfmr=2*lenr;
            for(i=l_pos;i<=l_end;++i)a[i]=add(a[i],tmpfz,tmpfml);
            for(i=r_start;i<=r_pos;++i)a[i]=add(a[i],tmpfz,tmpfmr);
            dw=1;
        }
        else if(!fl&&fr)
        {
