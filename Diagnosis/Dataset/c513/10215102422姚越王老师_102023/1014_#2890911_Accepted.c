...
int main()
{
	...
	in=a/b;a%=b;
	//printf("in=%d,a=%d,b=%d\n",in,a,b);
	int tern_int[100]={0},tern_point[100]={0};
	int length_int;
	length_int=int2tern(tern_int,in);
	//printf("int2tern:\n");
	...
	else if(length_int!=0&&length_point!=0)
	{
		int total[length_int+1+length_point];
		for(int k=1;k<length_int+1;k++)total[k]=tern_int[k-1];
	...
	{
		int total[length_point+1];
		for(int k=1;k<length_point+1;k++)total[k]=tern_point[k-1];
		add(total,length_point);
		sub(total,length_point);
		printf("%d.",total[0]);
		for(int i=1;i<=length_point;i++)printf("%d",total[i]);
		return 0;
	}
}