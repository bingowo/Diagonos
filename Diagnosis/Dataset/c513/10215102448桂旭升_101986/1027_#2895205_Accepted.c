...
SCORE* Input(int n)
{
    ...
    for(i = 0; i < n; i++){
		...
		if(d[0] < 0 && d[1] > 0){a[i].c = atan(1)*4 - fabs(atan(d[1]/d[0]));//printf("c = %lf\n", fabs(atan(d[1]/d[0])));
		}
		if(d[0] < 0 && d[1] < 0)a[i].c = atan(1)*4 + fabs(atan(d[1]/d[0]));
		if(d[0] > 0 && d[1] < 0)a[i].c = atan(1)*8 - fabs(atan(d[1]/d[0]));
		if(d[0] == 0 && d[1] == 0)a[i].c = 0;
		if(d[0] == 0 && d[1] > 0)a[i].c = atan(1)*2;
		if(d[0] == 0 && d[1] < 0)a[i].c = atan(1)*6;
		if(d[0] < 0 && d[1] == 0)a[i].c = atan(1)*4;
