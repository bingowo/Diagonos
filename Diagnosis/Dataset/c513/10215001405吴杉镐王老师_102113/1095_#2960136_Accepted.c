...
int main() {
    ...
    init(&B,0);
    init(&BASE,1);
    for(unsigned long i=l-1;i>1;i--)
    {
        int val=s[i]-'0';
        if(s[i]>='A'&&s[i]<='F') val=s[i]-'A'+10; //从个位开始提取数字
        for(int p=0;p<4;p++)
            if(val&(1<<p)) //判断-1+i进制下的整数
            {
                oper(&A,A,1,BASE,hash[p][0]);
                oper(&B,B,1,BASE,hash[p][1]);
            }
        multi(&BASE,-4);
    }
    ...
    if(B.cnt!=1 || B.num[1]!=0)
    {
        if(B.flg==-1)
            printf("-");
        else if(A.cnt!=1||A.num[1]!=0)
            printf("+");
        if(B.num[1]!=1 || B.cnt!=1)
        {
            printf("%lld",B.num[B.cnt]);
            for(int i=B.cnt-1; i; i--)
                printf("%08lld",B.num[i]);
        }
        putchar('i');
    }
    if(B.cnt==1 && B.num[1]==0 && A.cnt==1 && A.num[1] == 0)
        putchar('0');
    return 0;
}
