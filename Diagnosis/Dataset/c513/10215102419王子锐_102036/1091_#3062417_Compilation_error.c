...
BIGINT BIGMUL(BIGINT S , BIGINT T)  //两个大整数相乘
{
 ...
 {   int t,k,j;int carry=0;for(j=0;j<S.cnt;j++)
     {
...

{
 ...
     {
     i nt temp= (S.v[i] +T.v [i] +car ry) ;
         R.v[i] =temp%10 ;
         carry=temp/10 ;
     }
     while (i <S.cnt)
     {
         int temp=S . v [i ] +car ry ;
         R.v[i++]=temp%10 ;
         carry=temp/10 ;  }
     whi e (i<T.cnt)
     {
...
void print (COMPLEX * result)
{
 ...
     if( result->im.cnt!=0&&resul t->im.sign==1) printf("+") ; //存在虚部，而且虚部系数为正，输出+

 }
 if( resul t->i m . cnt ! =0) //存在虚部
