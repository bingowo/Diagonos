...
bool cmp(rows_data a, rows_data b){
    ...
    while (a.data_value[i] != -1 && b.data_value[i] != -1){
        if(a.data_value[i] > b.data_value[i]) return true;
        else if(a.data_value[i] < b.data_value[i]) return false;
        else i++;
    }
    if(a.data_value[i] == -1) return false;
    else return true;
}
...
void solve(int T){
    ...
    qsort(rowsData,N,sizeof(rows_data),cmp);
    for (int i = 0; i < N; ++i){
        for (int j = 0; j < 100; ++j){
            if(rowsData[i].data_value[j] == -1) break;
