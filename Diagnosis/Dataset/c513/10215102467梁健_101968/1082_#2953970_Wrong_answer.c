...
void solveint(char *A,char *B)
{
    ...
    for(int i=0;i<len+1;i++)res[i]=48;
    for(int i=0;i<len;i++){
        m=i;
        res[i]=A[i]+B[i]-48;
        while(res[m]>'9'){
    ...
    char cres[1000];
    for(int i=0;i<len1;i++)cres[i]=res[len1-i-1];
    strcpy(res,cres);
    int Len=strlen(res);
    int startp=0;
    for(int i=0;i<Len;i++){
    ...
    for(int i=startp;i<Len;i++)printf("%c",res[i]);
}
int main()
{
    char a[1000],b[1000];
