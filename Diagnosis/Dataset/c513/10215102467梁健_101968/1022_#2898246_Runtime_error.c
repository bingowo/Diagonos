...
int cmp(const void *a,const void *b)
{
    ...
    char un1[5000],un2[5000];
    for(int i=0;i<place1&&i<5000;i++)un1[i]=a1.username[i];
    for(int i=0;i<place2&&i<5000;i++)un2[i]=b1.username[i];
    char net1[2000];
    char net2[2000];
    for(int i=place1;i<strlen(a1.username)&&i<2000;i++)net1[i-place1]=a1.username[i];
    for(int i=place2;i<strlen(b1.username)&&i<2000;i++)net2[i-place2]=b1.username[i];
    int m=0;
    do{
        if(net1[m]!=net2[m]&&net1[m]!='\0'&&net2[m]!='\0')return (net1[m]>net2[m]?1:-1);
        if(net1[m]=='\0'&&net2[m]!='\0')return -1;
        if(net1[m]!='\0'&&net2[m]=='\0')return 1;
        m++;
