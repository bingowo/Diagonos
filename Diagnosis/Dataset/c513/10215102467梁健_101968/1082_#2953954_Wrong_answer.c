...
void solveint(char *A,char *B)
{
    char c[1000],d[1000];
    int m=0;
    int len1=strlen(A),len2=strlen(B),len=len1>len2?len1:len2;
    for(int i=0;i<len1;i++)c[len1-i-1]=A[i];
    strcpy(A,c);
    for(int i=0;i<len2;i++)d[len2-i-1]=B[i];
    strcpy(B,d);
    if(len==len1)for(int i=len2;i<len;i++)B[i]=48;
    else for(int i=len1;i<len1;i++)A[i]=48;
    for(int i=0;i<len;i++){
        m=i;
        A[i]+=(B[i]-48);
        while(A[m]>'9'){
            A[m]-=10;
            if(A[m+1]<='9'&&A[m+1]>='0')A[m+1]++;
            else A[m+1]='1';
            m++;
        }
    }
...
{
    char a[1000],b[1000];
    int flaga=1,flagb=1,dotpa=0,dotpb=0;
