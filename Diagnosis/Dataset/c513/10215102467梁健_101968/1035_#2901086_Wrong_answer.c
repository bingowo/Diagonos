...
int main()
{
    ...
    int judge1=0;//判断选的m-1个里面底面积是否是n个里面最大的。
    for(int i=0;i<n;i++)if(a[i].RS>RSmax)judge1+=1;
    if(judge1==0)qsort(a,n,sizeof(CYLIN),cmp);
    int judge2=0;
    for(int i=0;i<n;i++)if(a[i].RS+a[i].HS>RSmax)judge2+=1;
    if(judge2==0)qsort(a,n,sizeof(CYLIN),cmp);
