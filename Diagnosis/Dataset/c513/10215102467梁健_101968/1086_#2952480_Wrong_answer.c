...
int main()
{
    ...
    scanf("%s",a);scanf("%s",b);
    scanf("%d",&N);
    if(strchr(a,'.') == NULL){a[strlen(a)] = '.';a[strlen(a)] = '0';a[strlen(a)]=0;}
    if(strchr(b,'.') == NULL){b[strlen(b)] = '.';b[strlen(b)] = '0';b[strlen(b)]=0;}
    int dotpa=0,dotpb=0;
    ...
    for(int i=dotpa+1;i<strlen(a);i++){
        resA[i-dotpa-1]=a[i];
        if(i==strlen(a)-1)resA[i-dotpa]=0;
    }
    for(int i=dotpb+1;i<strlen(b);i++){
        resB[i-dotpb-1]=b[i];
    ...
    if(strlen(intA)>strlen(intB)||(strlen(intA)==strlen(intB)&&strcmp(intA,intB)>=0)){
        ...
        if(rlen>strlen(resA))for(int i=strlen(resA);i<rlen;i++)resA[i]='0';
        int k=0;
        if(N>strlen(resA))for(int i=strlen(resA);i<N;i++)resA[i]=48;
