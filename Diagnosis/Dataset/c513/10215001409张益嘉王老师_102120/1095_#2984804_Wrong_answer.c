...
bigint jianpu(bigint a,bigint b)
{
    ...
    int j,i;
    re.sign=1;
    memset(re.shu,0,sizeof(int)*1000);
    for(i=0;(i<a.wei)||(i<b.wei);i++)
    {
        if(a.shu[i]>=b.shu[i]) re.shu[i]=a.shu[i]-b.shu[i];
        else
        {
...
fushu jia(fushu re,int c)
{
    ...
    else
    {
        ...
        else
        {
            ...
            re.shibu.shu[i]--;
            if((re.shibu.shu[i]==0)&&(i==(re.shibu.wei-1))) re.shibu.wei--;
...
int main()
{
    char s[100];
    scanf("%s",s);
    int c[400],i,j,len,lc;
    fushu re;
    memset(re.shibu.shu,0,sizeof(int)*1000);
    memset(re.xubu.shu,0,sizeof(int)*1000);
    //memset(flag.shibu.shu,0,sizeof(int)*1000);
    //memset(flag.xubu.shu,0,sizeof(int)*1000);

    r0.shibu.sign=1;
    r0.xubu.sign=1;
    memset(r0.shibu.shu,0,sizeof(int)*1000);
    memset(r0.xubu.shu,0,sizeof(int)*1000);
    r0.shibu.wei=1;
    r0.xubu.wei=1;
