...
int main()
{
    ...
    for(i=0;i<T;i++)
    {
        ...
        for(j=0;j<n;j++)
        {
            ...
            if(dian[j].x==0)
            {
                if(dian[j].y>0) dian[j].thr=M_PI/2.0;
                else if (dian[j].y<0) dian[j].thr=-(M_PI/2.0);
                else dian[j].thr=0;
            }
            else dian[j].thr=atan(dian[j].y/dian[j].x);
            if(dian[j].thr<0)
            {
                if((dian[j].x>0)&&(dian[j].y<0))dian[j].thr=-dian[j].thr;
                else if((dian[j].x<0)&&(dian[j].y>0)) dian[j].thr+=M_PI;
            }
            else if(dian[j].thr>0)
            {
                if((dian[j].x<0)&&(dian[j].y<0)) dian[j].thr+=M_PI;
            }
            else
            {
                if(dian[j].x>=0) dian[j].thr=0;
                else if (dian[j].x<0) dian[j].thr=M_PI;
            }
