...
void jiayi(int*c2,int *c1,int wei,int maxf,int maxz)
{
    ...
    fz(c2,maxf);
    int k=0;
    if(maxz>1){while(c1[k]==0) k++;
    for(int i=k;i<maxz;i++)
    printf("%d",c1[i]);}
    if(maxz==1) printf("%d",c1[0]);
    if(maxz==0) printf("0");
    if(wei>0)
       {
           printf(".");
...
int main()
{
    ...
    int flag=0;
    if(z1<z2) {jh(a1,a2,z1,z2);jh(a2,b2,f1,f2);flag=1;}
    if(z1==z2)
    {
        if(strcmp(a1,b1)<0) {jh(a1,a2,z1,z2);jh(a2,b2,f1,f2);flag=1;}
        if(strcmp(a1,b1)==0)
        {
            if(f1<f2) {jh(a1,a2,z1,z2);jh(a2,b2,f1,f2);flag=1;}
            if(f1==f1&&strcmp(a2,b2)<0) {jh(a1,a2,z1,z2);jh(a2,b2,f1,f2);flag=1;}
        }
    }
