...
int main()
{
    ...
    while((scanf("%s",s))!=EOF)
    {
        if(strstr(s,"IN"))
        {
            ...
            if(s[k1]=='-') {flag1=-1;k1++;}
            while(s[k1]>='0'&&s[k1]<='9'&&s[k1]!='\0') {a=0;a=a*10+s[k1]-48;k1++;}
            a=a*flag1;}
            if(strstr(s,"BX"))
             {int k2=0;
            while(s[k2]<'0'||s[k2]>'9'&&s[k2]!='-') k2++;
            if(s[k2]=='-') {flag2=-1;k2++;}
            while(s[k2]>='0'&&s[k2]<='9'&&s[k2]!='\0') {b=0;b=b*10+s[k2]-48;k2++;}
            b*=flag2;}
            if(strstr(s,"CX"))
             {int k3=0;
            while(s[k3]<'0'||s[k3]>'9'&&s[k3]!='-') k3++;
            if(s[k3]=='-') {flag3=-1;k3++;}
            while(s[k3]>='0'&&s[k3]<='9'&&s[k3]!='\0') {c=0;c=c*10+s[k3]-48;k3++;}
            c*=flag3;}
            if(strstr(s,"DX"))
             {int k4=0;
            while(s[k4]<'0'||s[k4]>'9'&&s[k4]!='-') k4++;
            if(s[k4]=='-') {flag4=-1;k4++;}
            while(s[k4]>='0'&&s[k4]<='9'&&s[k4]!='\0') {d=0;d=d*10+s[k4]-48;k4++;}
            d*=flag4;}
        }
        ...
        {
            scanf("%s",s);
            if(strstr(s,"AX")) printf("%lld",a);
            if(strstr(s,"BX")) printf("%lld",b);
            if(strstr(s,"CX")) printf("%lld",c);
            if(strstr(s,"DX")) printf("%lld",d);
        }
