...
zb*INPUT(int m)
{
    ...
    for(int k=0;k<m;k++)
    {
        ...
        {
          if(s[k].zong>0&&s[k].heng>0) s[k].jiao=pai/4.0;
          if(s[k].zong>0&&s[k].heng<0) s[k].jiao=3*pai/4.0;
          if(s[k].zong<0&&s[k].heng>0) s[k].jiao=7*pai/4.0;
          if(s[k].zong<0&&s[k].heng<0) s[k].jiao=5*pai/4.0;
        }
        if(s[k].heng==0&&s[k].zong!=0) 
        {
            if(s[k].zong>0) s[k].jiao=pai/2.0;
            else s[k].jiao=3*pai/2.0;
        }
        else if(s[k].zong==0)
        {
