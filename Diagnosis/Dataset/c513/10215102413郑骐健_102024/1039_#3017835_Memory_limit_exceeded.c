...
var apl(int l,int r)//返回表达式的结果，cnt为-1则表示返回的是变量的索引
{
    ...
    {
    ar = apl(j+2,r);
    if(strcmp(item[l],"+")==0)return addL(ar);
    if(strcmp(item[l],"-")==0)return subL(ar);
    if(strcmp(item[l],"*")==0)return mulL(ar);
    }
    else
    {
         ar = apl(j+1,r);
...
var sub(var al,var ar)
{
    var cmp;
      if(al.cnt ==1)
        {
          cmp = ar;
          for(int i =0;i<ar.cnt;i++)cmp.var[i] = al.var[0] - ar.var[i];
        }
    else if(ar.cnt ==1)
        {
          cmp = al;
          for(int i =0;i<al.cnt;i++)cmp.var[i] -= ar.var[0];
        }
    return cmp;
}
var addL(var m)
