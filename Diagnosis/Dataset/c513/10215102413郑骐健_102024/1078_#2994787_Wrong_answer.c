...
void pourwater(number* p, int s, int pos, int n);
void full(number* p, number half, int l_pos, int r_pos, int dir);
...
//返回平地灌水后剩余的水量
int water_lowest(number* p, int s, int pos, int l_pos, int r_pos)
{
...
number full_next(number* p, number half, int l_pos, int r_pos, int dir)
{
    ...
    int v = 0;
    for (int i = l_pos + dir;i != t_pos+dir;i += dir)
    {
        if (p[i].v < high) v += high - p[i].v;
    }
    if (half.v >= v)
    {
        for (int q = l_pos + dir;q != t_pos+dir;q += dir)
        {
...
void full_final(number* p, number v, int l_pos, int r_pos, int dir)
{
    ...
    for (int i = l_pos + dir;i != r_pos + dir;i += dir)
    {
        if (p[i].v <= high)
        {
            p[i].v = high;
