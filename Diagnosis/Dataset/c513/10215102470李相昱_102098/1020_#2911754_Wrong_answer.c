...
int main() {
    ...
    while (n!=0){
        ...
        for (int i = 0; i < n; i++) {
            ...
            a[i].size = 0;
            int q=0;
            for(;q< strlen(a[i].all)&&(!isdigit(a[i].all[q]));q++);
            while(isdigit(a[i].all[q++])){
                a[i].size = a[i].size *10+a[i].all[q]-'0';
            }
            int m=0;
            q = q+1;
            for(;q< strlen(a[i].all);m++,q++){
                a[i].name[m] = a[i].all[q];
            }
            a[i].name[m] = 0;
        }
        scanf("LIST /%s",s);
        ...
        for(int j=0;j<n;j++){
            printf("%d-%s-%s %s:%s%s\n", a[j].year, a[j].month,
                   a[j].day, a[j].hour, a[j].min, a[j].all);
        }
        scanf("%d",&n);
        if(n!=0){
            printf("\n");
