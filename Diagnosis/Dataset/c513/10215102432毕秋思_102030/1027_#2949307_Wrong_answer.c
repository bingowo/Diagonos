...
double my_atan(double x,double y)
{
    if(x>0)
    {
        ...
        else return PI*2.0-atan(fabs(y/x));
    }
    if(x<0)
    {
        if(y>0) return PI-atan(fabs(y/x));
        else return PI+atan(fabs(y/x));
    }
    if(x==0)
    {
        if(y>0) return PI/2.0;
        else if(y<0) return PI*3.0/2.0;
    }
}
...
int main()
{
    ...
    for(int t=0;t<T;t++)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            points[i].ang=my_atan(points[i].x,points[i].y);
