...
int cmp(const void* a,const void* b){
    ...
    if(l1<l2)return -1;//前l1位均相等
    else return 1;
}

int main(){
    char r[27],s[5000];
    int space[110];//记录空格位置
    memset(r,0,sizeof(r));memset(s,0,sizeof(s));
    while(gets(r)!=NULL){
        gets(s);
        for(int i=0;i<110;i++)space[i]=0;
        for(int i=0;i<27;i++)R[i]=0;//初始化
        for(int i=0;i<26;i++)R[r[i]-'A']=i;
