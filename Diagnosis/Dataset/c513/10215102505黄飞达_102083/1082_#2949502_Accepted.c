...
int main(){
    ...
    if (p==NULL) for (int i=0; i<lena; i++) ax[i]=a[lena-1-i];
    else {
        pos = p-a;
        //倒置ax
    ...
    p = strstr(b, ".");
    if (p==NULL) for (int i=0; i<lenb; i++) bx[i]=b[lenb-1-i];
    else{
        pos = p-b;
        //倒置bx
