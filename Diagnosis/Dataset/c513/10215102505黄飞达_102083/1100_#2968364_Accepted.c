...
int main(){
    ...
    for (int q=0; q<Q; q++){
        ...
        for (int i=0;i<len; i++){
            ...
            if (sum_x==0 && sum_y!=0){
                if (x0==a && (b-y0)%sum_y==0 && (b>y0&&sum_y>0 || b<y0&&sum_y<0)) {printf("Yes\n");flag=1;break;}
                
            }
            else if (sum_x!=0 && sum_y==0){
                if (y0==b && (a-x0)%sum_x==0 && (a>x0&&sum_x>0 || a<x0&&sum_x<0)) {printf("Yes\n");flag=1;break;}
            }
            else if (sum_x!=0&&sum_y!=0){
                if ((a-x0)/sum_x==(b-y0)/sum_y && (a-x0)%sum_x==0 && (b-y0)%sum_y==0) {
