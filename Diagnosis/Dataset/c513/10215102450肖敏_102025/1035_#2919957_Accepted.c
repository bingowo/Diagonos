...
int cmp(const void *a, const void *b) {
    if (((long long *) a)[0] ==
        ((long long *) b)[0]) { if (((long long *) a)[2] <= ((long long *) b)[2]) return -1; else return 1; }
    else if (((long long *) b)[0] < ((long long *) a)[0]) return -1;
    else
...
int main() {
    ...
    for (int j = 0; j < m; j++) sum += a[j][2];
    qsort(a, m, sizeof(a[0]), cmp);
    long long r_min = a[0][2];
    for (int l = 1; l < m; l++) { if (a[l][2] < r_min) r_min = a[l][2]; }
    long long temp = 0;
    for (int k = m; k < n; k++) {
        if (a[k][0] >= a[0][0] && (a[k][2] + a[k][0] * a[k][0]) > (r_min + a[0][0] * a[0][0]) &&
            a[k][2] + a[k][0] * a[k][0] > temp) { temp = a[k][2] + a[k][0] * a[k][0]; }
    }
    if (temp) {
        sum -= r_min;
        sum += temp;
