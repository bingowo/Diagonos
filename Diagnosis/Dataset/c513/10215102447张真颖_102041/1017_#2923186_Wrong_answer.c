...
int main()
{
    ...
    for(int i=0;i<strlen(s);i++)
    {
       if(s[i]=='(') mul*=1000,sign=0;
        ...
        else
        {
            ...
            {
                if(arr[s[i]]==arr[s[i+1]]||arr[s[i]]==arr[s[i-1]])
                {
                    sum+=arr[s[i]];
                }
                else if((arr[s[i]]==1||arr[s[i]]==10||arr[s[i]]==100)&&arr[s[i+1]]>arr[s[i]]&&i+1<strlen(s))
                    sum-=arr[s[i]];
    ...
    for(int i=0;i<t;i++)
    {
        ...
        else if((at[i]==1||at[i]==10||at[i]==100)&&at[i+1]>at[i]&&i+1<t)
        {
            r-=at[i];
        }
