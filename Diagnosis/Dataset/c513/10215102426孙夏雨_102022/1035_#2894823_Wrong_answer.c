...
int cmp3(const void* p3,const void* p4)
{
    AAA* p1=(AAA* )p3;
    AAA* p2=(AAA* )p4;
    if(((p1->RR)+(p1->RH)*2)<((p2->RR)+(p2->RH)*2)) return 1;
    else return -1;
}
...
int main()
{
    ...
    long long int smax=p->RR,a1=0;
    for (int j=0;j<m;j++) smax+=2*((p+j)->RH);
    for (int jj=0;jj<m-1;jj++) a1+=2*((p+jj)->RH);
    qsort(p+m,n-m,sizeof(AAA),cmp3);
    a1+=(p->RR)+2*(p->RH);
    printf("%lld",smax>a1 ? smax:a1);
    free(p);
    return 0;
}
