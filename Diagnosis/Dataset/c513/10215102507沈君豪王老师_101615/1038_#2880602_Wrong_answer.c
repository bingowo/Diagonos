...
int main(void)
{
	...
	lli area, SmallRect, LargeRect;
	for(int i = 0 ;i < n ; i ++){
		for(int j = 0 ; j < m ; j ++){
			for(int t = 0; i + t < n && j + t < m; t ++){
				area = SmallRect = LargeRect = 0;
				...
				if(t != 0 && t != 1){
					if(i == 0 && j == 0){
						SmallRect = in[i + t][j + t].SumOfMatrix;
					}else if(i == 0){
						SmallRect = in[i + t][j + t].SumOfMatrix - in[i + t][j - 1].SumOfMatrix;
					}else if(j == 0){
						SmallRect = in[i + t][j + t].SumOfMatrix - in[i - 1][j + t].SumOfMatrix;
					}else{
						SmallRect = in[i + t][j + t].SumOfMatrix - in[i + t][j - 1].SumOfMatrix - in[i - 1][j + t].SumOfMatrix + in[i - 1][j - 1].SumOfMatrix;
					}
					if(SmallRect - area >= k){ cnt ++;}					
				}
				if(i + t < n - 1 && j + t < m - 1){
					if(i == 0 && j == 0){
			...
			for(int t = 0; i + t < n && j - t >= 0; t ++){
				...
				if(t != 0 && t != 1){
					if(i == 0 && j - t == 0){
						SmallRect = in[i + t][j].SumOfMatrix;
					}else if(i == 0){
						SmallRect = in[i + t][j].SumOfMatrix - in[i + t][j - t - 1].SumOfMatrix;
					}else if(j - t == 0){
						SmallRect = in[i + t][j].SumOfMatrix - in[i - 1][j].SumOfMatrix;
					}else{
						SmallRect = in[i + t][j].SumOfMatrix - in[i + t][j - t - 1].SumOfMatrix - in[i - 1][j].SumOfMatrix + in[i - 1][j - t -1].SumOfMatrix;
					}
					if(SmallRect - area >= k){ cnt ++;}					
				}
				if(i + t < n - 1 && j - t > 0){
