...
llu distance(Point *a, Point *b)
{
	...
	if((a->x < 0 && b->x > 0) || (a->x > 0 && b->x < 0)){
		ret1 = llabs(a->x) + llabs(b->x);
	}else{
		ret1 = llabs(a->x - b->x);
	}
	if((a->y < 0 && b->y > 0) || (a->y > 0 && b->y < 0)){
		ret2 = llabs(a->y) + llabs(b->y);
	}else{
		ret2 = llabs(a->y - b->y);
	}
	if(ret1 > LLONG_MAX && ret2 > LLONG_MAX){
		ret = -1;
	}else{
		ret = ret1 + ret2;
	}
...
int main(void)
{
	...
	if(n == 2 && distance(&in[0], &in[1]) == -1){
		printf("18446744073709551616\n0");	//Ê²Ã´¹íÊý¾ÝÄãÔõÃ´¸øÕâ¸ö±íÊ¾²»³öÀ´µÄÄÑµÀÎªÁËÄãÕâ¸öÊý¾Ý»¹ÒªÐ´Ò»¸ö´óÊý¼Ó·¨Âð£¿ 
	}else{
		for(int i = 0; i < n - 1; i ++){
