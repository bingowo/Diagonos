...
int main(){
    ...
    while(s){
        ...
        if (left_l >= p) right_s  = s, left_s = 0;
        else if (right_l <= p) left_s = s, right_s = 0;

        if (left_l == right && right_l == left){
            if (left_need <= s) for (int i = left; i <= right; i++) groud[i]++, s--;
            else{
                lfm = left_need, lfz = s; break;
            }
        }
        else{
    ...
    for (int i = 1; i <= n; i++){
        if (groud[i]) printf("%d", groud[i]);
        else if (groud[i] == 0 && !(i >= left && i <= left_l || i >= right_l && i <= right)) printf("0");
        if (lfz != 0 && i >= left && i <= left_l) printf("%c%d/%d", groud[i]? '+' : '\0', lfz / gcd(lfm, lfz), lfm / gcd(lfm, lfz));
