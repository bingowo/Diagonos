...
BIG mul(BIG T, BIG S){
    ...
    while(temp1.Rcnt > 0 && temp1.Rv[temp1.Rcnt - 1] == 0) temp1.Rcnt--;
    ...
    while(temp2.Rcnt > 0 && temp2.Rv[temp2.Rcnt - 1] == 0) temp2.Rcnt--;
    ...
    while(temp3.Icnt > 0 && temp3.Iv[temp3.Icnt - 1] == 0) temp3.Icnt--;
    ...
    while(temp4.Icnt > 0 && temp4.Iv[temp4.Icnt - 1] == 0) temp4.Icnt--;
...
int main(){
    ...
    BIG t = ItoB(s);
    BIG res = POW(t, exp);
    if (!res.rp && res.Rcnt) printf("-"); 
    for (int i = res.Rcnt - 1; i >= 0; i--)
        printf("%d", res.Rv[i]);
    if (res.Rcnt != 0 && res.Icnt != 0) printf("%c", res.ip? '+': '-');
    else if (res.Rcnt == 0 && res.Icnt != 0) printf("%c", res.ip? '+': '-');   
