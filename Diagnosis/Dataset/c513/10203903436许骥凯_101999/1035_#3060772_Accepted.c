...
unsigned long long cal_area(cylinder *arr, int m, unsigned long long max_r){
...
unsigned long long exchange(cylinder *arr, int lo, int hi, unsigned long long max_r, unsigned long long res){
    for (int i = lo; i != hi; ++i){
        if (arr[i].R <= max_r) continue;;
        unsigned long long dis = arr[i].area - max_r * max_r;
...
unsigned long long find_max(cylinder* arr, int m){
    unsigned long long res = 0;
    for (int i = 0; i != m; ++i)
...
unsigned long long process(cylinder *arr, int n, int m){
    ...
    unsigned long long cyl_bottom_r = find_max(arr, m);
    unsigned long long res = cal_area(arr, m, cyl_bottom_r);
    qsort(arr + m, n - m, sizeof(cylinder), cmp_by_R_A);
