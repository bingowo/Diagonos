#include<stdio.h>
#include<stdlib.h>
#define HALF_MAX_ULL 9223372036854775808ULL
...
unsigned long long sort(point *arr, int n, int *flag){
    qsort(arr, n, sizeof(point), cmp);
    unsigned long long x = cal_dis(arr[0].x, arr[1].x);
    unsigned long long y = cal_dis(arr[0].y, arr[1].y);
    if (x == HALF_MAX_ULL && y == HALF_MAX_ULL){
        *flag = 1;
        return 0;
    }
...
int main(){
    ...
    Input(arr_pt, n);
    int flag = 0;
    unsigned long long res = sort(arr_pt, n, &flag);
    if (flag) printf("18446744073709551616\n");
    else printf("%llu\n", res);
    printf("%d\n", cal_step(arr_pt, n));
    return 0;
}
