...
int main()
{
	...
	if(ar[0].x==ar[1].x&&ar[0].y==ar[1].y) sd=llabs(ar[0].x-ar[1].x)+llabs(ar[0].y-ar[1].y);
	else sd=llabs(ar[0].x-ar[1].x)-1+llabs(ar[0].y-ar[1].y);
	//printf("%llu\n",sd);
	
	while(sd!=1&&((sd==-1&&i==0)||(sd+1)%2!=0||((sd+1)/2)%2!=0)&&i<n-1)
	{
		int m=0;
		//printf("k=%llu\n",sd);
		...
		//printf("%d %llu\n",m,sd);
		i++;
		if(ar[i].x==ar[i+1].x&&ar[i].y==ar[i+1].y) sd=llabs(ar[i].x-ar[i+1].x)+llabs(ar[i].y-ar[i+1].y);
		else sd=llabs(ar[i].x-ar[i+1].x)-1+llabs(ar[i].y-ar[i+1].y);
